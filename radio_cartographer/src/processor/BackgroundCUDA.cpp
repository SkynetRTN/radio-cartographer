...
std::vector<double> Baseline::autoFixedWRegression()
{
    ...
    if (linearModel || noModel)
    {
        ...
        {
            std::cout << "[ WARNING ] Quad Fixed Regression Failed in a Worrysome Way" << std::endl;
        }

        if (trueCount >= 2 && noModel)
        {
            std::cout << "[ WARNING ] All Fixed Regression Failed in a Worrysome Way" << std::endl;
        }
    }
...
void Baseline::setLocalModels()
{
    ...
    for (int k = low; k < high + 1; k++)
    {
        ...
        if (!((holderBVec[0] > BLFlux[k]) && (BLChecks[k] == 0)))
        {
            ...
            {
                std::cout << "[ WARNING ] Bad Local Model." << std::endl;
                bgWeight = NAN;
            }
            else if (counter == 1)
            {
