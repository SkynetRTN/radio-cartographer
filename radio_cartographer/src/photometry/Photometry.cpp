...
Photometry::Photometry(Composite composite, MapParameters mParams)
{
...
// Callers
void Photometry::photometry(Map &mapHold, PhotometryParameters pParams)
{
    // Map
    map = mapHold;
...
void Photometry::photometerMulti(Map &mapHold, PhotometryParameters pParams)
{
    // Map
    map = mapHold;
    ...
    case CentroidMethods::COORDINATES:
        ...
        for (int i = 0; i < coordinatesFull.size() / 2; i++)
        {
            futureVec[i] = std::async(std::launch::async, &Photometry::photometer, this, i, coordinatesFull, dim);
        }
        for (int i = 0; i < coordinatesFull.size() / 2; i++)
        {
...
std::vector<double> Photometry::photometer(int index, std::vector<double> coordinates, Dimensions dim)
{
    // Declarations
...
// Determiners
void Photometry::determineDimensions(std::vector<double> coords, Dimensions &dim, PhotometryParameters pParams)
{
    // Define edge of map
...
void Photometry::determinePixelParameters(double &maxPixRa, double &maxPixDec, double &centPixDec, double &centPixRa)
{
...
double Photometry::determineAnnulusSigma(double mu, double sigma)
{
    double sigma_C;
...
double Photometry::determineAnnulusMu(std::vector<double> f, std::vector<double> w)
{
...
std::vector<double> Photometry::determineCorrFactor(double peakSum, double peakRadius, double thetaAperPix, double sigma)
{
    // Declarations
...
std::vector<std::vector<double>> Photometry::determineNRValues(std::vector<double> r, std::vector<double> d, std::vector<double> f, std::vector<double> w, std::vector<bool> flags)
{
    // Declaration
...
std::vector<double> Photometry::annulusCalculations(Dimensions dim, std::vector<double> center)
{
    // Declarations
    int raPix, decPix;
    double w, w2;
...
std::vector<double> Photometry::apertureCalculations(Dimensions dim, std::vector<double> center, double annulusMu)
{
...
// Calculations
void Photometry::photometryMetrics(Dimensions dim, std::vector<double> anParams, std::vector<double> apParams, std::vector<double> &metrics)
{
...
std::vector<double> Photometry::sigmaPlus(double mu, std::vector<std::vector<double>> nrValues)
{
...
// Centroids
std::vector<double> Photometry::findBrightestPixel()
{
    int maxDec, maxRa;
...
std::vector<double> Photometry::autoCentroid(int yPixel, int xPixel, double searchRadius)
{
    // yPixel is dec pixel
    // xPixel is ra pixel
...
std::vector<double> Photometry::determineCenters(int pointCount, std::vector<double> &inRange, double scale, double i_0, double j_0)
{
    ...
    {
        std::cout << "[ WARNING ] Centroiding unsuccessful; forcing original coordinates." << std::endl;
        coordinates[0] = i_0;
        coordinates[1] = j_0;
    }
