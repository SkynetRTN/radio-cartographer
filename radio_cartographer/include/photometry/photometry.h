...
{
public:
	Photometry(Composite, MapParameters);

	PartitionSet compPartSetProc;
	std::vector<Scan> scans;

	// Callers
	void photometry(Map &, PhotometryParameters);
	std::vector<double> photometer(int, std::vector<double>, Dimensions);

	// Multi-Threaded Callers
	void photometerMulti(Map &, PhotometryParameters);
...
private:
	...
	void determineDimensions(std::vector<double>, Dimensions &, PhotometryParameters);
