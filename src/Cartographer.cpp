...
Map Cartographer::generateProcMapsMulti() {
  procMap.reserveGrid(compPartSetProcSSS.maxDec, compPartSetProcSSS.minDec,
                      compPartSetProcSSS.maxRa, compPartSetProcSSS.minRa,
                      resolution, compPartSetProcSSS.centerDecDeg,
                      compPartSetProcSSS.centerRaDeg);

  // INTERPOLATES THE GRID FOR ALL PIXELS

  int totalPixelsDec =
      (int)(compPartSetProcSSS.maxDec - compPartSetProcSSS.minDec) /
      (resolution);
  int totalPixelsRa =
      (int)(compPartSetProcSSS.maxRa - compPartSetProcSSS.minRa) / (resolution);
  ...
  for (double i = compPartSetProcSSS.minDec; i <= compPartSetProcSSS.maxDec;
       i += resolution) {
    xPixel = Tools::determinePixel(i, compPartSetProcSSS.minDec, resolution);
    completedThreads = 0;
    counter = 0;
    futureVec.resize(
        (int)((compPartSetProcSSS.maxRa - compPartSetProcSSS.minRa) /
              resolution) +
        1);
    for (double j = compPartSetProcSSS.minRa; j <= compPartSetProcSSS.maxRa;
         j += resolution) {
      yPixel = Tools::determinePixel(j, compPartSetProcSSS.minRa, resolution);

      futureVec[counter] = std::async(
          std::launch::async, &Cartographer::processedGridInterpolationMulti,
          this, i, j);
      liveThreads++;
      counter++;
  ...
  for (int i = 0; i < scans.size(); i++) {
    for (int j = 0; j < scans[i].getSize(); j++) {
      a = Tools::min((double)procMap.getSize(0) - 1,
                     round((1.0 / resolution) *
                           (scans[i].getDec(j) - compPartSetProcSSS.minDec)));
      b = Tools::min((double)procMap.getSize(1) - 1,
                     round((1.0 / resolution) *
                           (scans[i].getRa(j) - compPartSetProcSSS.minRa)));
...
std::vector<double> Cartographer::processedGridInterpolationMulti(double i_0,
                                                                  double j_0) {
  MapTypes mapType;
  ...
  if (SSSMapping) {
    ...
    scaleSSS = Tools::max(
        processingWeightingFunction,
        Tools::min((4.0 / 3.0) * largestGapSSS / (psfFWHM * toRad), 1.0));
    corrSSS = getCorrMapValueSSS(i_0, j_0, inRange, scaleSSS);
    fromSolve = applySurfaceModel(modelType, false, m10PlusCriteria, inRange,
                                  largestGapSSS, i_0, j_0);
  ...
  if (LSSMapping) {
    ...
    scaleLSS = Tools::max(
        processingWeightingFunction,
        Tools::min((4.0 / 3.0) * largestGapLSS / (psfFWHM * toRad), 1.0));
    corrLSS = getCorrMapValueLSS(i_0, j_0, inRange, scaleLSS);

    fromSolve = applySurfaceModel(modelType, true, false, inRange,
                                  largestGapLSS, i_0, j_0);
    fluxLSS = fromSolve[0];
    wLSS = fromSolve[1];
    w2LSS = fromSolve[2];
    if (modelType == M0) {
...
void Cartographer::setMapValues(int xPixel, int yPixel,
                                std::vector<double> &results) {
...
// misc
int Cartographer::findPossSSS(double i_0, double j_0, double limit,
  ...
  // BEFORE PRECEEDING UNDERSTAND CLASSIFY() IN SURVEY.CPP FOR DEFINITIONS ON
  // subDecInc AND subDecRes subDecInc IS THE NUMBER OF DEGREES NEEDED TO BE
  // PAST TO BE IN A PARTITION SET RANGE
  int decCounter =
      (int)((decTemp - partSetProc.minDec) / partSetProc.subDecInc);
  ...
  for (int i = Tools::max(decCounter - 1, 0);
       i < Tools::min((double)decCounter + 2, partSetProc.subDecRes); i++) {
    for (int j = Tools::max(RACounter - 1, 0);
         j < Tools::min((double)RACounter + 2, partSetProc.subRaRes); j++) {
      for (int k = 0; k < classificationsSSS[i][j].size(); k += 2) {
        ...
        if ((i_0 - limit <= decHold) && (decHold <= i_0 + limit) &&
            (j_0 - limit <= raHold) && (raHold <= j_0 + limit))
        /*
...
int Cartographer::findPossLSS(double i_0, double j_0, double limit,
                              std::vector<int> &toRet) {
  // TAKES IN INDICIES FOR SPECIFIC POINT
  std::vector<int> intFiller;
  toRet.resize(100, 0.0);
...
double Cartographer::getPixelThetaGapSSS(double i_0, double j_0,
                                         std::vector<double> &inRange) {
  int holdI, holdJ;
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    distance = Tools::getGCDistance(i_0, j_0, scans[holdI].getDec(holdJ),
                                    scans[holdI].getRa(holdJ),
                                    compPartSetProcSSS.centerDecDeg);
    dataWeightScale = scans[holdI].getThetaGap(holdJ);

    exponent = -2.329267 * log(((dataWeightScale / 2.0) / (psfFWHM * toRad))) -
               0.510417;
...
double Cartographer::getPixelThetaGapLSS(double i_0, double j_0,
                                         std::vector<double> &inRange) {
  int holdI, holdJ;
  double dataWeightScale, dataWeight, wySum = 0, wSum = 0, exponent;
  double toRad = M_PI / 180.0, toDeg = 180.0 / M_PI, distance;
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    distance = Tools::getGCDistance(i_0, j_0, scans[holdI].getTSDec(holdJ),
                                    scans[holdI].getTSRa(holdJ),
                                    compPartSetProcSSS.centerDecDeg);
...
double Cartographer::getCorrMapValueSSS(double i_0, double j_0,
                                        std::vector<double> &inRange,
                                        double scaleMapVal) {
  int holdI, holdJ;
  double factor_w = 3.0;
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    wSum += dataWeight;
    wySum += sqrt(pow(scans[holdI].getSSSCorrelation(holdJ) / psfFWHM, 2) +
                  pow(factor_w * scaleMapVal, 2)) *
             dataWeight;
  }

  return wySum / wSum;
}
double Cartographer::getCorrMapValueLSS(double i_0, double j_0,
                                        std::vector<double> &inRange,
                                        double scaleMapVal) {
  int holdI, holdJ;
  double factor_w = 3.0;
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    wSum += dataWeight;
    wySum += factor_w *
             sqrt(pow(scans[holdI].getLSSThetaWPrime(holdJ), 2) +
                  pow(scaleMapVal, 2)) *
             dataWeight;
  }
...
std::vector<double> Cartographer::collectPointsSSS(double i_0, double j_0) {
  ...
  for (int i = 0; i < pointCandidates; i++) {
    ...
    if (distance < psfFWHM) {
      ...
      // distance = Tools::getGCDistance(scans[holdI].getDec(holdJ),
      // scans[holdI].getRa(holdJ), medianDec, medianRa);
      count++;
    }
  }
...
std::vector<double> Cartographer::collectPointsLSS(double i_0, double j_0) {
  ...
  for (int i = 0; i < pointCandidates; i++) {
    ...
    distance = Tools::getGCDistance(i_0, j_0, scans[holdI].getLSSDec(holdJ),
                                    scans[holdI].getLSSRa(holdJ),
                                    compPartSetProcSSS.centerDecDeg) *
               toDeg;
...
Cartographer::determineScanNumbersSSS(std::vector<double> &inRange) {
  std::vector<int> intFiller;
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    holdJ = inRange[6 * i + 5];
    contained = false;
    // PUSH BACK VALUES OF SCAN NUMBER AND DATA POINT ON THAT SCAN INTO
    // scanNumbers VECTOR
...
std::vector<std::vector<int>>
Cartographer::determineScanNumbersLSS(std::vector<double> &inRange) {
  std::vector<int> intFiller;
...
bool Cartographer::checkEdgeCriteria(double ra, double dec) {
  ...
  coordinateCheck = determineMappedEdges(ra, dec, tracking, compPartSetProcSSS,
                                         pCoordinate, mapType);
...
std::vector<double>
Cartographer::determineMappedEdges(double LONG, double LATI, bool tracking,
...
// surface modeling
bool Cartographer::planeApplication(int minScanCount, int minPointsInScan,
                                    int minPointsInAp,
...
SurfaceType
Cartographer::determineSurfaceType(std::vector<std::vector<int>> &scanNumbers,
                                   MapTypes mapType) {
  SurfaceType modelType;
...
std::vector<double> Cartographer::applySurfaceModel(
    SurfaceType modelType, bool largeStructure, bool m10PlusCriteria,
  ...
  case M0:
    ...
    for (int i = 0; i < inRange.size() / 6; i++) {
      w += inRange[6 * i + 1];
      wLocalSum += inRange[6 * i + 1] *
                   scans[inRange[6 * i + 4]].getGMWeight(inRange[6 * i + 5]);
    }
...
std::vector<double> Cartographer::surfaceFit3(bool m10PlusCriteria,
                                              std::vector<double> &inRange,
                                              double largestGap, double i_0,
                                              double j_0) {
  ...
  weightingFunctionTemp =
      Tools::max(weightingFunction,
                 Tools::min((4.0 / 3.0) * largestGap / (psfFWHM * toRad), 1.0));
  double alpha = (log(0.5)) / (log(cos(M_PI * weightingFunctionTemp / 4.0)));

  // Photometry variables
  double rfiDistance, correlation = -999999, wcTop = 0.0, wcBottom = 0.0,
                      wCorrelation;
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    if (rfiScaleDeg != 0.0 && correlatedWeightMap == true) {
      correlation = scans[inRange[6 * i + 4]].getSSSCorrelation(
          inRange[6 * i + 5]); // in deg?

      for (int j = 0; j < inRange.size() / 6; j++) {
        rfiDistance =
            Tools::getGCDistance(inRange[6 * i + 2], inRange[6 * i + 3],
                                 inRange[6 * j + 2], inRange[6 * j + 3],
                                 compPartSetProcSSS.centerDecDeg) *
            toDeg;
        if (rfiDistance < correlation / 2.0) // refining photometry is as simple
                                             // as tweaking this number.
        {
          intersect += 1;
        }
      }
    ...
    if (rfiScaleDeg != 0 && correlatedWeightMap == true) {
      wLocalW += wHold *
                 scans[inRange[6 * i + 4]].getGMWeight(inRange[6 * i + 5]) /
                 intersect;
      // wLocalW += pow(scans[inRange[6 * i + 4]].getGMWeight(inRange[6 * i +
      // 5]), beta);
    } else {
      wLocalW += wHold *
                 scans[inRange[6 * i + 4]].getGMWeight(inRange[6 * i + 5]) *
                 scans[inRange[6 * i + 4]].getDataDumps(inRange[6 * i + 5]);
    }
...
std::vector<double> Cartographer::surfaceFit6(bool m10PlusCriteria,
                                              std::vector<double> &inRange,
                                              double largestGap, double i_0,
                                              double j_0) {
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    if (rfiScaleDeg != 0.0 && correlatedWeightMap == true) {
      correlation =
          scans[inRange[6 * i + 4]].getSSSCorrelation(inRange[6 * i + 5]);
      for (int j = 0; j < inRange.size() / 6; j++) {
        rfiDistance =
            Tools::getGCDistance(inRange[6 * i + 2], inRange[6 * i + 3],
                                 inRange[6 * j + 2], inRange[6 * j + 3],
...
std::vector<double> Cartographer::surfaceFit10(bool m10PlusCriteria,
                                               std::vector<double> &inRange,
                                               double largestGap, double i_0,
                                               double j_0) {
  ...
  if (m10PlusCriteria == false &&
      m10PlusProcessing == true) // ONLY OCCURS IN RECURSIVE CALL
  {
    A[0] = 2 * w;
  }
  ...
  if (finalAnswer < 0.0 && m10PlusCriteria == true) {
    ...
    m10PlusCriteria = false;
    m10PlusValues =
        surfaceFit10(m10PlusCriteria, inRange, largestGap, i_0, j_0);
