...
PreProcessor::PreProcessor() { this->hiRes = false; }

class PointToPointFunc : public NonParametric {
public:
  PointToPointFunc(std::vector<double>, std::vector<double>);

  void muFunc(std::vector<bool> &, std::vector<int> &, std::vector<double> &,
              std::vector<double> &);
  void muFunc(std::vector<bool> &, std::vector<int> &, std::vector<double> &,
...
PreProcessor::sdfitsReader(SpectralParameters cParams) {
  ...
  if (telescope == "GreenBank-20" || telescope == "NRAO20") {
    ...
    averageSpectra(spectra.getFrequencies(), cParams.inclusionBand,
...
// Input Data
int PreProcessor::accessExtTable20m(
    std::vector<valarray<double>> &valArraySpectra, Receiver receiver,
    int file) {
  // Declarations
  ...
  std::unique_ptr<CCfits::FITS> pInfile(
      new CCfits::FITS(filename[file - 1], CCfits::Read, hdu, false));
  CCfits::ExtHDU &table = pInfile->extension(hdu);

  columns = {"UTSECS", "CRVAL2",   "CRVAL3",   "AZIMUTH",  "ELEVATIO",
  ...
  if (file == 1) {
    ...
    for (int i = 0; i < columns.size(); i++) {
      inputData[i].insert(inputData[i].end(), inputTemp[i].begin(),
                          inputTemp[i].end());
    }
    valArraySpectra.insert(valArraySpectra.end(), vasTemp.begin(),
                           vasTemp.end());
  }
...
int PreProcessor::accessExtTableGBT(int file) {
  ...
  columns = {"",       "CRVAL2", "CRVAL3",   "AZIMUTH", "ELEVATIO",
             "NSAMPS", "",       "PROCSEQN", "",        ""};
  inputTemp.resize(columns.size());
...
int PreProcessor::primaryHeader() {
  ...
  std::unique_ptr<CCfits::FITS> pInfile(
      new CCfits::FITS(name, CCfits::Read, true));
...
void PreProcessor::getHistoryInfo(const std::string history) {
  ...
  // Although history info is shown as multiple rows in header, it is single
  // string
...
void PreProcessor::performCleaningMulti(Spectra spectra, double baseline) {
  ...
  for (int i = 0; i < spectra20.size(); i++) {
    ...
    futureVec[i] = std::async(std::launch::async, &BackgroundCUDA::calculateBG,
                              scCuda, baseline);
    counter++;
    liveThreads++;
...
void PreProcessor::determineScatterMulti(Spectra &spectra) {
  ...
  for (int i = 0; i < spectra20.size(); i++) {
    ...
    futureVec[i] = std::async(std::launch::async,
                              &PreProcessor::calculateScatter, this, i);
    counter++;
    liveThreads++;
...
void PreProcessor::formatSpectra20(std::vector<valarray<double>> vaSpec) {
  ...
  for (int i = 0; i < vaSpec.size();
       i++) { // Convert from valarray to vector -- revese to match frequencies
    std::copy(std::begin(vaSpec[i]) + intRanges[0] - 1,
              std::begin(vaSpec[i]) + intRanges[1], vSpecHold.begin());
...
void PreProcessor::formatGBTInput(std::string mode, std::vector<string> dates,
...
// Misc
int PreProcessor::averageSpectra(std::vector<std::vector<double>> frequencies,
...
void PreProcessor::sortLowResData() {
  ...
  //{
  //	std::transform(outputData[1].begin(), outputData[1].end(),
  // outputData[1].begin(),
...
void PreProcessor::determineTelescope() {
  ...
  std::unique_ptr<CCfits::FITS> pInfile(
      new CCfits::FITS(filename[0], CCfits::Read, true));
  CCfits::PHDU &image = pInfile->pHDU();
...
void PreProcessor::appendColumnData() {
  ...
  //// Open sdfits file and open its binary table. This is risky. We are editing
  /// original data files!
  // std::unique_ptr<CCfits::FITS> pInfile(new CCfits::FITS(filename,
  // CCfits::Write, hduName, false)); CCfits::ExtHDU& table =
  // pInfile->extension(hduName);
  ...
  // for (int i = 0; i < spectra20.size(); i++)
  //{
  //	std::copy(spectra20[i].begin(), spectra20[i].end(),
  // std::begin(valSpectrum)); 	valSpectra20.push_back(valSpectrum);
...
void PreProcessor::validateSystem() {
  if (system == "forced_equatorial") {
    system = "equatorial";
    throw "warning: No coordinate system info was found in the header.. The "
          "observation is assumed to be mapped in equatorial coordinates..\n";
  }
}
...
// Getters
int PreProcessor::getMJDate() { return this->MJDate; }
bool PreProcessor::getScanningDirection() { return this->scanningDirection; }
double PreProcessor::getYear() { return this->year; }
double PreProcessor::getFrequency() { return this->frequency; }
std::string PreProcessor::getTelescope() { return this->telescope; }
std::string PreProcessor::getMapPattern() { return this->pattern; }
std::string PreProcessor::getCoordinateSystem() { return this->system; }
...
void Spectra::determineFrequency(double fStep, double obsFreq,
                                 double centerOffset) {
...
void Spectra::keepBackground(std::vector<std::vector<double>> &bgOutput,
                             double freqStep) {
  int numOfPoints;
  double delta;
...
void Spectra::insertPoints(std::vector<std::vector<double>> &bgOutput,
                           double freqStep, int i, int j,
                           std::string position) {
  if (position == "end") {
    ...
    if (position == "start") {
      ...
      freqDists[i].insert(freqDists[i].begin() + j,
                          freqDists[i][j - 1] + freqStep);
      frequencies[i].insert(frequencies[i].begin() + j,
                            frequencies[i][j - 1] + freqStep);
    }
...
// Getters
double Spectra::getScatter(int i) { return this->scatters[i]; }
std::vector<double> Spectra::getFlux(int i) { return this->flux[i]; }
std::vector<double> Spectra::getWeight(int i) { return this->weights[i]; }
std::vector<double> Spectra::getFreqDist(int i) { return this->freqDists[i]; }
std::vector<double> Spectra::getBaseline(int i) { return this->baselines[i]; }
std::vector<std::vector<double>> Spectra::getFreqDists() {
  return this->freqDists;
}
std::vector<std::vector<double>> Spectra::getWeights() { return this->weights; }
...
void Spectra::setScatter(double value, int i) { this->scatters[i] = value; }
