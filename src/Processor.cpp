...
std::vector<std::vector<double>> spawnNewRCRThread(std::vector<double> w,
                                                   std::vector<double> y) {
...
Processor::Processor(ProcessorParameters &procParams) {
  ...
  skipTS = procParams.skipTS;
  skipBG = procParams.skipBG;
  skipRFI = procParams.skipRFI;
}
...
void Processor::performBGSubtractionMulti(Survey &survey) {
  ...
  if (bgScaleBW != 0.0 && !skipBG) {
    for (int i = 0; i < scans.size(); i++) {
      Debugger::print("Info", i);
      bgCuda = BackgroundCUDA(scans[i], false);
      futureVec[i] =
          std::async(std::launch::async, &BackgroundCUDA::calculateBGMulti,
                     bgCuda, bgScaleBW * psfFWHM);
      counter++;
      liveThreads++;
...
void Processor::performTimeShifting(Survey &survey) {
  ...
  double t_int = 0;
  int isTs = 0;
  if ((timeShift == "auto" || timeShift == "custom") && !skipTS) {
    isTs = 1;
    if (timeShift == "auto" || !timeShiftValue) {
      ProcessorTS procTS = ProcessorTS(scans, mapType, psfFWHM,
                                       medianDiffAlongSweeps, scansInRa);
...
void Processor::set2DScatter(Survey &survey) {
  ...
  double minAngle1, maxAngle1, minAngle2, maxAngle2, minAngle3, maxAngle3,
      angleHold, minAngle, maxAngle, toPush, weight, weightLow, weightHigh,
      xBar, angleHigh, angleLow, angleMid, weightMid;
  std::vector<double> angleVec1, angleVec2, angleVec3, delta, weights,
      scatter2DAverages;
  ...
  for (int i = 1; i < scans.size() - 1; i++) {
    ...
    maxAngle3 = -999999;
    if (mapType == DAISY) {
      angleVec1 = Tools::daisyAngleBuilder(
          i - 1, scans[i - 1].getCenter(), partSetProcSSS.centerDecDeg,
          scans[i - 1].getRa(), scans[i - 1].getDec());
      angleVec2 = Tools::daisyAngleBuilder(i, scans[i].getCenter(),
                                           partSetProcSSS.centerDecDeg,
                                           scans[i].getRa(), scans[i].getDec());
      angleVec3 = Tools::daisyAngleBuilder(
          i + 1, scans[i + 1].getCenter(), partSetProcSSS.centerDecDeg,
          scans[i + 1].getRa(), scans[i + 1].getDec());
    } else {
      if (scansInRa) {
    ...
    if (angleVec2[angleVec2.size() - 1] > angleVec2[0]) {
      ...
      for (; j < k; j++) {
        ...
        if (jBefore < angleVec1.size() - 1 &&
            std::abs(angleVec1[jBefore] - angleVec2[j]) >
                std::abs(angleVec1[jBefore + 1] - angleVec2[j])) {
          jBefore++;
        }
        while (angleVec3[jAfter] < angleVec2[j]) {
          jAfter--;
        }
        if (jAfter < angleVec3.size() - 1 &&
            std::abs(angleVec3[jAfter] - angleVec2[j]) >
                std::abs(angleVec3[jAfter + 1] - angleVec2[j])) {
          jAfter++;
        }
        if (scansInRa) {
          toPush =
              scans[i].getFlux(j) -
              (scans[i - 1].getFlux(jBefore) +
               (scans[i + 1].getFlux(jAfter) - scans[i - 1].getFlux(jBefore)) /
                   (scans[i + 1].getDec(jAfter) -
                    scans[i - 1].getDec(jBefore)) *
                   (scans[i].getDec(j) - scans[i - 1].getDec(jBefore)));
          ...
          toPush =
              ...
              (scans[i - 1].getFlux(jBefore) +
               (scans[i + 1].getFlux(jAfter) - scans[i - 1].getFlux(jBefore)) /
                   (scans[i + 1].getRa(jAfter) - scans[i - 1].getRa(jBefore)) *
                   (scans[i].getRa(j) - scans[i - 1].getRa(jBefore)));
          delta.push_back(toPush);
        ...
        xBar = (weightLow * angleLow + weightHigh * angleHigh) /
               (weightLow + weightHigh);
        weight = (1.0 / scans[i].getDataDumps(j));
        weight +=
            pow(((angleHigh - xBar) * (angleHigh - angleLow)), 2.0) / weightLow;
        weight +=
            pow(((angleLow - xBar) * (angleHigh - angleLow)), 2.0) / weightHigh;
        weight += pow(((angleMid - xBar) * (angleHigh - angleLow)), 2.0) *
      ...
      for (; j > k; j--) {
        ...
        if (jAfter > 0 && std::abs(angleVec3[jAfter] - angleVec2[j]) >
                              std::abs(angleVec3[jAfter - 1] - angleVec2[j])) {
          jAfter--;
        }
        if (scansInRa) {
...
void Processor::performRFIRejectionMulti(Composite &composite,
  ...
  if (rfiScaleBW != 0.0 && !skipRFI) {
    ...
    ProcessorRFI processorRFI(scans, rfiParams, scatter2dSSS,
                              classificationsSSS);
    ...
    for (int i = 0; i < scans.size(); i++) {
      scans[i].removeRFI(
          rfiSubtracted[i]); // resets the working channel
                             // scans[i].removePoints(flags[i]); // removes
                             // flagged points (this is also used in trimming
                             // edges, so we might be able to just rewrite this
                             // as a clip RFI Data Function that takes in the
                             // RFISubtractedVector
      scans[i].updateAngDistTemp(partSetProcSSS.centerDecDeg);
    }
  }
...
void Processor::calculateProcThetaGapMulti(Composite &composite) {
  ...
  ProcessorThetaGap processorTG(scans, psfFWHM, partSetProcSSS, partSetProcLSS,
                                classificationsSSS, classificationsLSS);
  ...
  for (int i = 0; i < scans.size(); i++)
  {
          ...
          {
                  futureVec[j] = std::async(std::launch::async,
  &Processor::circleThetaGap, this, i, j, false);
          }
          for (int j = 0; j < scans[i].getSize(); j++)
          {
  ...
  if (LSSProcessing)
  {
          for (int i = 0; i < scans.size(); i++)
          {
                  ...
                  {
                          futureVec[j] = std::async(std::launch::async,
  &Processor::circleThetaGap, this, i, j, LSSProcessing);
                  }
                  for (int j = 0; j < scans[i].getLSSSize(); j++)
                  {
                          value = futureVec[j].get();
...
void Processor::determineDataBoundaries(Survey &survey) {
  ...
  // IF OVERLAYING IMAGES, EACH SURVEY SHOULD BE CENTERED ABOUT 0 WHEN DEC
  // TRANSFORMED
  ...
  if (mapType != DAISY) {
    ...
    for (int i = 0; i < scans.size(); i++) {
      for (int j = 0; j < scans[i].getSize(); j++) {
        // scans[i].cosDecTransform(0, survey.getMedianDec(),
        // survey.getMedianRa(), survey.getMedianDec());
        radii.push_back(
            Tools::getGCDistance(scans[i].getDec(j),
                                 scans[i].getRa(j) * cos(survey.getMedianDec()),
                                 survey.getMedianDec(), survey.getMedianRa(),
                                 survey.getMedianDec()) *
            toDeg); // THIS IS WRONG: DAISYS ARE DAISIES WHEN IN RA*C0S(DEC)
                    // SPACE, BUT WHEN STILL RAW RA AND DEC, THEY ARE ELONGATED
                    // ALONG THE RA AXIS, SO WE ARE TRYING TO UNSTRETCH THEM
                    // TWICE... scans[i].undoCosTransform(survey.getMedianDec(),
                    // survey.getMedianDec(), survey.getMedianRa());
      }
    }
  ...
  survey.setPartSetProcSSS(
      partSetProcSSS); // WE ONLY NEED SSS BECAUSE LSS CHANGES ONLY ONCE WE'VE
                       // REJECTED POINTS FROM SSS
}
void Processor::characterizeData(Survey &survey) {
  ...
  for (int i = 0; i < scans.size(); i++) {
    ...
    //{
    //	scans[i].cosDecTransform(survey.getTimeShift(), survey.getMedianDec(),
    // survey.getMedianRa(), survey.getMedianDec());
    //	scans[i].updateAngDistTemp(survey.getMedianDec());
    // }
    // else
    //{
    //	scans[i].cosDecTransform(survey.getTimeShift(), globalCenterDec,
    // globalCenterRa, globalCenterDec);
    //	scans[i].updateAngDistTemp(globalCenterDec);
    // }

    if (survey.getTracking()) {
      if (survey.getProcessingCoordinate() == EQUATORIAL) {
        scans[i].dynamicCosDecTransform(
            survey.getTimeShift(), survey.getMedianDec(), survey.getMedianRa(),
            scans[i].getDec());
        scans[i].updateAngDistTemp(survey.getMedianDec());
      } else {
        scans[i].dynamicCosDecTransform(
      ...
      if (survey.getProcessingCoordinate() == EQUATORIAL) {
        scans[i].dynamicCosDecTransform(survey.getTimeShift(), globalCenterDec,
                                        globalCenterRa, scans[i].getDec());
        scans[i].updateAngDistTemp(globalCenterDec);
      } else {
        scans[i].dynamicCosDecTransform(survey.getTimeShift(), globalCenterDec,
                                        globalCenterRa, scans[i].getDec());
  ...
  if (survey.getTracking()) {
    ...
    partSetProcSSS.medianRa =
        (survey.getMedianRa() - globalCenterRa) * cos(globalCenterDec);
  }
...
// tools
int Processor::findPossSSS(double i_0, double j_0, double limit,
  ...
  int pointCounter = 0,
      decCounter =
          (int)((decTemp - partSetProcSSS.minDec) / partSetProcSSS.subDecInc),
      RACounter =
          (int)((RATemp - partSetProcSSS.minRa) / partSetProcSSS.subRaInc),
      iHold, jHold;
  // int pointCounter = 0;
  // int decCounter = round(((decTemp - partSetProc.minDec) /
  // partSetProc.subDecInc)); int RACounter = round((RATemp - partSetProc.minRa)
  // / partSetProc.subRaInc); int iHold, jHold;
  ...
  // for (int i = Tools::max(decCounter - 1, 0); i <
  // Tools::min((double)decCounter + 2, partSetProc.subDecRes); i++)
  for (int i = Tools::max(decCounter - 1, 0);
       i < Tools::min((double)decCounter + 2, partSetProcSSS.subDecRes); i++) {
    // for (int j = Tools::max(RACounter - 1, 0); j <
    // Tools::min((double)RACounter + 2, partSetProc.subRaRes); j++)
    for (int j = Tools::max(RACounter - 1, 0);
         j < Tools::min((double)RACounter + 2, partSetProcSSS.subRaRes); j++) {
      for (int k = 0; k < classificationsSSS[i][j].size(); k += 2) {
        iHold = classificationsSSS[i][j][k];
        jHold = classificationsSSS[i][j][k + 1];

        distance = Tools::getGCDistance(i_0, j_0, scans[iHold].getDec(jHold),
                                        scans[iHold].getRa(jHold),
                                        partSetProcSSS.centerDecDeg) *
                   toDeg;

        // if ((i_0 - limit <= scans[iHold].getDec(jHold))
        //	&& (scans[iHold].getDec(jHold) <= i_0 + limit)
        //	&& (j_0 - limit <= scans[iHold].getRa(jHold))//THESE ARE ALREADY
        // COSINE TRANSFORMED??
        //	&& (scans[iHold].getRa(jHold) <= j_0 + limit))

        //&& (j_0*cos(i_0*toRad) - limit <=
        // scans[iHold].getRa(jHold)*cos(i_0*toRad))//THESE ARE ALREADY COSINE
        // TRANSFORMED??
        //&& (scans[iHold].getRa(jHold)*cos(i_0*toRad) <= j_0*cos(i_0*toRad) +
        // limit))
...
int Processor::findPossLSS(double i_0, double j_0, double limit,
                           std::vector<int> &toRet) {
  std::vector<int> intFiller;
  toRet.resize(100, 0);
  double decTemp = i_0, RATemp = j_0, toRad = M_PI / 180.0;
  // int pointCounter = 0, decCounter = (int)((decTemp - partSetProc.minDec) /
  // partSetProc.subDecInc), RACounter = (int)((RATemp - partSetProc.minRa) /
  // partSetProc.subRaInc), iHold, jHold;
  int pointCounter = 0;
  int decCounter =
      round(((decTemp - partSetProcLSS.minDec) / partSetProcLSS.subDecInc));
  int RACounter =
      round((RATemp - partSetProcLSS.minRa) / partSetProcLSS.subRaInc);
  int iHold, jHold;
  ...
  // for (int i = Tools::max(decCounter - 1, 0); i <
  // Tools::min((double)decCounter + 2, partSetProc.subDecRes); i++)
  for (int i = Tools::max(decCounter - index, 0);
       i < Tools::min((double)decCounter + index + 1, partSetProcLSS.subDecRes);
       i++) {
    // for (int j = Tools::max(RACounter - 1, 0); j <
    // Tools::min((double)RACounter + 2, partSetProc.subRaRes); j++)
    for (int j = Tools::max(RACounter - index, 0);
         j < Tools::min((double)RACounter + index + 1, partSetProcLSS.subRaRes);
         j++) {
      for (int k = 0; k < classificationsLSS[i][j].size(); k += 2) {
        ...
        if ((i_0 - limit <= scans[iHold].getDec(jHold)) &&
            ...
            (j_0 - limit <=
             scans[iHold].getRa(
                 jHold)) // THESE ARE ALREADY COSINE TRANSFORMED??
            && (scans[iHold].getRa(jHold) <= j_0 + limit))
...
void Processor::classifyLSS(Survey &survey) {
  ...
  for (int i = 0; i < scans.size(); i++) {
    ...
    for (int j = 0; j < scans[i].getLSSSize(); j++) {
      ...
      decTemp = decHold[j];
      decCounter =
          (int)((decTemp - partSetProcLSS.minDec) / partSetProcLSS.subDecInc);
      raCounter =
          (int)((raTemp - partSetProcLSS.minRa) / partSetProcLSS.subRaInc);
      if (decCounter <= 0) {
        decCounter = 0;
      }
      if (raCounter <= 0) {
...
PartitionSet Processor::determineProcSurveyDimensions(Survey &survey,
                                                      bool LSS) {
  ...
  double decTemp, raTemp;
  partSetProc.minDec = 999999, partSetProc.minRa = 999999,
  partSetProc.maxDec = -999999, partSetProc.maxRa = -999999;
  ...
  if (partSetProc.mapType == DAISY) {
    ...
    for (int i = 0; i < scans.size(); i++) {
      ...
      for (int j = 0; j < size; j++) {
        // distance = Tools::getGCDistance(scans[i].getDec(j),
        // scans[i].getRa(j), partSetProc.medianDec, partSetProc.medianRa,
        // partSetProc.centerDecDeg)*toDeg;//LAST TERM SHOULD PROBABLY BE 0
        // DAISY PATHS ARE CORRECTED THIS SHOULD NO LONGER BE MOD
        if (LSS) {
          distance = Tools::getGCDistance(
                         scans[i].getLSSDec(j), scans[i].getLSSRa(j),
                         partSetProc.centerDecDeg, partSetProc.centerRaDeg,
                         partSetProc.centerDecDeg) *
                     toDeg;
        } else {
          distance = Tools::getGCDistance(scans[i].getDec(j), scans[i].getRa(j),
                                          partSetProc.centerDecDeg,
                                          partSetProc.centerRaDeg,
                                          partSetProc.centerDecDeg) *
                     toDeg;
        }
    ...
    Debugger::print("Info", "Edge Radius in BW",
                    partSetProc.edgeRadius / psfFWHM);
  }

  // If mapped in equatorial but want to process in galactic
  if (survey.getMappingCoordinate() == "equatorial" &&
      survey.getProcessingCoordinate() == GALACTIC) {
  ...
  else if (survey.getMappingCoordinate() == "galactic" &&
           survey.getProcessingCoordinate() == EQUATORIAL) {
  ...
  partSetProc.centerLatProcDeg =
      globalCenterLatProc; // never used if coordinate == MAPPED.
  ...
  partSetProc.subDecRes =
      Tools::max(1, (int)((partSetProc.maxDec - partSetProc.minDec) /
                          Tools::max(psfFWHM * rfiScaleBW, psfFWHM)));
  partSetProc.subRaRes =
      Tools::max(1, (int)(((partSetProc.maxRa - partSetProc.minRa) /
                           Tools::max(psfFWHM * rfiScaleBW, psfFWHM)) *
                          Tools::min(cos(partSetProc.maxDec / toDeg),
                                     cos(partSetProc.minDec / toDeg))));

  partSetProc.subDecInc =
      (partSetProc.maxDec - partSetProc.minDec) / partSetProc.subDecRes;
  partSetProc.subRaInc =
      (partSetProc.maxRa - partSetProc.minRa) / partSetProc.subRaRes;

  partSetProc.medianLatiMap =
      medianLatiMapped; // never used if tracking == false.
  partSetProc.medianLongMap = medianLongMapped;

  return partSetProc;
}
PartitionSet Processor::determineProcCompositeDimensions(Composite &composite,
                                                         bool LSS) {
  PartitionSet partSetProc;
  scans = composite.getScans();
  ...
  if (composite.getMappingCoordinate() == "equatorial" &&
      composite.getProcessingCoordinate() == GALACTIC) {
    globalCenterLatProc = Tools::convertToB(globalCenterRa, globalCenterDec);
    globalCenterLongProc = Tools::convertToL(globalCenterRa, globalCenterDec);
  } else if (composite.getMappingCoordinate() == "galactic" &&
             composite.getProcessingCoordinate() == EQUATORIAL) {
    globalCenterLatProc = Tools::convertToDec(globalCenterRa, globalCenterDec);
    globalCenterLongProc = Tools::convertToRa(globalCenterRa, globalCenterDec);
  }
...
void Processor::repartitionScans(Composite &composite,
                                 std::vector<Survey> &surveys) {
...
std::vector<bool> Processor::shiftRejection(std::vector<double> shifts) {
  double minAngle1, maxAngle1, minAngle2, maxAngle2, angleHold, minAngle,
      maxAngle, angleStep, maxIFFT, hold;
  ...
  for (int i = 0; i < scans.size() - 1; i++) {
    ...
    maxAngle2 = -999999;
    if (mapType == DAISY) {
      angleVec1 = Tools::daisyAngleBuilder(i, scans[i].getCenter(),
                                           partSetProcSSS.centerDecDeg,
                                           scans[i].getRa(), scans[i].getDec());
      angleVec2 = Tools::daisyAngleBuilder(
          i + 1, scans[i + 1].getCenter(), partSetProcSSS.centerDecDeg,
          scans[i + 1].getRa(), scans[i + 1].getDec());
    } else {
      if (scansInRa) {
  ...
  for (int i = 0; i < shifts.size(); i++) {
    ...
    if (M == 1) {
      deltaJ = shifts[j];
      probHold = (std::abs(deltaI - deltaJ) / R) *
                 (2 - (std::abs(deltaI - deltaJ) / R));
      prob = pow(2, (M - 1)) * probHold;
    } else {
      j = i + 1;
      deltaJ = shifts[j];
      probHold1 = (std::abs(deltaI - deltaJ) / R) *
                  (2 - (std::abs(deltaI - deltaJ) / R));

      j = i - 1;
      deltaJ = shifts[j];
      probHold2 = (std::abs(deltaI - deltaJ) / R) *
                  (2 - (std::abs(deltaI - deltaJ) / R));
...
double
Processor::calculateShift(std::vector<std::vector<double>> &maxIfftInfo) {
  RCR rcr = RCR(LS_MODE_DL);
  bool stop = false, first = true, alreadySorted = false;
  double muBelow, muAbove, stDevAbove, stDevBelow, belowAboveRatio,
      maxBelowAboveRatio = -999999;
  double maxBelowAboveIndex, shiftWeightSum = 0.0, weightSum = 0.0;
  double result;
  int counter2 = 0;
  std::vector<bool> checks, shiftRejectionFlags, flagHold;
  std::vector<double> swap, shiftsAbove, shiftsBelow, weightTemps, shifts,
      weights, tempArray, tempWeights, tempArray2, tempWeights2, weightsBelow,
      weightsAbove, ratioAbove, ratioBelow;
...
Processor::getScanToScanShifts(double sampling) {
  ...
  for (int i = 0; i < scans.size() - 1; i++) {
    ...
    maxAngle = std::min(maxAngle1, maxAngle2);
    angleStep =
        std::min((maxAngle1 - minAngle1) / (double(scans[i].getSize())),
                 (maxAngle2 - minAngle2) / (double(scans[i + 1].getSize()))); //
    requiredPower = 1;
    ...
    if (angleVec1[angleVec1.size() - 1] > angleVec1[0]) {
      ...
      for (int k = 0; k < pow(2, requiredPower); k++) {
        ...
        if (angleVec1[h] == angleVec1[h - 1]) {
          ...
          interpolated[1].push_back(
              scans[i].getFlux(h - 1) +
              (angleHold - angleVec1[h - 1]) *
                  (scans[i].getFlux(h) - scans[i].getFlux(h - 1)) /
                  (angleVec1[h] - angleVec1[h - 1]));
        }
        angleHold += angleStep;
      }
      angleHold = std::max(minAngle1, minAngle2);
      ...
      for (int k = 0; k < pow(2, requiredPower); k++) {
        ...
        if (angleVec2[h] == angleVec2[h + 1]) {
          ...
          interpolated[2].push_back(
              scans[i + 1].getFlux(h + 1) +
              (angleHold - angleVec2[h + 1]) *
                  (scans[i + 1].getFlux(h) - scans[i + 1].getFlux(h + 1)) /
                  (angleVec2[h] - angleVec2[h + 1]));
        }
        angleHold += angleStep;
      }
    } else {
    ...
    for (int j = 0; j < ifftResult.size(); j++) {
      hold = sqrt(ifftResult[j][0] * ifftResult[j][0] +
                  ifftResult[j][1] * ifftResult[j][1]);
      if (hold > maxIFFT) {
        maxIFFT = hold;
        maxIFFTBin = j;
      }
...
void Processor::lssDropCorrection(Survey &survey) {
  ...
  // stop pushing back the pre and post indices and instead just use them to
  // call the continued iterations of 2D Scatter
  RCR rcr = RCR(SS_MEDIAN_DL);
  scans = survey.getScans();
      ...
      for (int j = 0; j < scans[i].getSize(); j++) {
        if (scans[i].getDropFlag(j) == 1 || scans[i].getDropFlag(j) == -1) {
          ...
          if (i == 0) {
            ...
            // preIndices.push_back(Tools::determineNearestIndex(j, holdAng1,
            // holdAng2)); // CHANGE TO BE DEC OR RA
            // proIndices.push_back(Tools::determineNearestIndex(j, holdAng1,
            // holdAng3));
          }
        }
...
void Processor::lssThetaGapCalc(Survey &survey) {
  ...
  scans = survey.getScans();
  ProcessorThetaGap procTG =
      ProcessorThetaGap(scans, psfFWHM, partSetProcSSS, partSetProcLSS,
                        classificationsSSS, classificationsLSS);
  ...
  for (int i = 1; i < scans.size() - 1; i++) {
    ...
    if (angleVec2[angleVec2.size() - 1] > angleVec2[0]) {
      ...
      for (; j < k; j++) {
        ...
        if (scansInRa) {
          toPush = scans[i].getLSSData(j) -
                   (scans[i - 1].getLSSData(jBefore) +
                    (scans[i + 1].getLSSData(jAfter) -
                     scans[i - 1].getLSSData(jBefore)) /
                        (scans[i + 1].getDec(jAfter) -
                         scans[i - 1].getDec(jBefore)) *
                        (scans[i].getDec(j) - scans[i - 1].getDec(jBefore)));
...
void Processor::lssPolynomialRFISubtraction(Survey &survey, double bgScale,
                                            double wScale) {
  bool multi = true;
  ...
  // cleanedLSSGrid[58][9] = lssRecursivePolyFit(survey, 120, 40, bgScale,
  // wScale); cleanedLSSGrid[58][9] = lssRecursivePolyFit(survey, 59, 10,
  // bgScale, wScale);
  ...
  if (multi) {
    ...
    for (int i = 0; i < scans.size(); i++) {
      ...
      for (int j = 0; j < scans[i].getSize(); j++) {
        futureVec[j] =
            std::async(std::launch::async, &Processor::lssRecursivePolyFit,
                       this, std::ref(survey), i, j, bgScale, wScale);
      }
      for (int j = 0; j < scans[i].getSize(); j++) {
    ...
    for (int i = 0; i < scans.size(); i++) {
      for (int j = 0; j < scans[i].getSize(); j++) {
        cleanedLSSGrid[i][j] =
            lssRecursivePolyFit(survey, i, j, bgScale, wScale);
      }
    }
  }
...
void Processor::lssElevationSubtraction(Survey &survey) {
  ...
  futureVec.resize(scans.size());
  scans = survey.getScans();
  calculateSurveyElevationScatter(); // LOOKS LIKE THIS IS 2D BACKGROUND AGAIN
                                     // INSTEAD OF THIS CALCULATE SCATTER V2
...
// lss drop
std::vector<double>
Processor::determinePercentDrop(std::vector<int> &preIndices,
                                std::vector<int> &proIndices) {
  ...
  for (int i = 0; i < scans.size(); i++) {
    for (int j = 0; j < scans[i].getSize(); j++) {
      if (scans[i].getDropFlag(j) == 1 || scans[i].getDropFlag(j) == -1) {
        ...
        for (int k = proIndices[indexCounter2]; k < maxProTemp;
             k++) // WE NEED A maxProTemp BECAUSE WE CAN'T CALL scans[i+1] SO WE
                  // JUST PROVIDE A FILLER
        {
        ...
        */
        ...
        if (percentRejected > max) // && (i != 0 && i != scans.size()-1))// did
...
void Processor::setDropFlags(std::vector<double> &delta) {
  ...
  // FIND THE AVERAGE DEVIATION FROM NOISE LEVEL AND THE LARGEST AND SMALLEST
  // NON-REJECTED POINTS
  RCR rcr = RCR(SS_MEDIAN_DL);
  rcr.performBulkRejection(delta);
  ...
  // IDENTIFY REJECTED POINTS (1), THOSE WITH DIFFERENCES BUT NOT REJECTED (0),
  // AND THOSE THAT HAVE NO DIFFERENCES (-1)
  for (int i = 0; i < scans.size(); i++) {
    for (int j = 0; j < scans[i].getSize(); j++) {
      if (scans[i].getDrop2dValues(j) != -999999) {
        if ((scans[i].getDrop2dValues(j) > maxVal) ||
            (scans[i].getDrop2dValues(j) < minVal)) {
...
void Processor::correctDrop(std::vector<double> maxDetails) {
  ...
  outFile << "Indices:\t" << maxScan << "\t" << maxData << "\t"
          << scans[maxScan].getSize() << "\n";
  ...
  // std::cout << "Indices:\t" << maxScan << "\t" << maxData << "\t" <<
  // scans[maxScan].getSize() << "\n";
...
int Processor::correctIndex(int i1, int j1, int i2,
                            int j2) // int j1, int j2, int i1, int i2)
{
  bool incWithIndex;
...
void Processor::calculateElevationBG(int scanIndex, double baseline) {
  ...
  // FIND THE MINIMUM AND MAXIMUM INDEX VALUES FOR A BASELINE LENGTH STARTING AT
  // INDEX start
  for (; start < size - 2; start++) {
    ...
    if (end != start) {
      // calculateBaselines(true, start, end, size, elevationScatter, elevation,
      // LSSData, dumps, bgData, bgWeights, ra, dec);
    }
  }
  end = size - 1;
  start = size - 1;
  for (; end > 1; end--) {
    ...
    if (start != end) {
      // calculateBaselines(false, start, end, size, elevationScatter,
      // elevation, LSSData, dumps, bgData, bgWeights, ra, dec);
    }
  }
  ...
  for (int i = 0; i < bgData.size(); i++) {
    ...
    if (trueCount == 0) {
      ...
      for (int j = 0; j < bgData[i].size(); j++) {
        if (bgData[i][j] == NAN || bgData[i][j] != bgData[i][j] ||
            bgWeights[i][j] == NAN || bgWeights[i][j] != bgWeights[i][j]) {
          trueCount--;
        } else {
...
// lss rfi
bool Processor::verifyOutlierState(SurfaceType modelType, int i_0, int j_0,
                                   double thetaFWHM, std::vector<double> &coef,
  ...
  double secondDerivative =
      calculateSecondDerivitive(modelType, i_0, j_0, thetaFWHM, coef);
...
bool Processor::planeApplication(int minScanCount, int minPointsInScan,
                                 int minPointsInAp,
                                 std::vector<std::vector<int>> scanNumbers,
                                 int counter, MapTypes mapType) {
...
void Processor::rejectModelPoints(int i_0, int j_0, double thetaMinPrime,
                                  double thetaFWHM, double avgScatter,
                                  std::vector<double> &inRange,
                                  std::vector<bool> &checks) {
  double largestGap, thetaWPrime, alpha;
  double secondDeriv, thetaPerp, signedDelta;
  double sigma = 999999, delta, max = -999999, xPrime, yPrime, zPrime, wPrime,
         thetaPrime;
  ...
  largestGap =
      scans[i_0].getLSSThetaGap(j_0); // circleThetaGap(i_0, j_0, true);

  while (sigma > avgScatter) {
    // largestGap = lssCircleThetaGap(i_0, j_0, inRange, checks);// in radians
    thetaWPrime = Tools::max(
        thetaMinPrime,
        Tools::min(((4.0 / 3.0) * largestGap * toDeg / psfFWHM), thetaFWHM));
    alpha = -1.0 * std::log(2) /
            (std::log(std::cos(M_PI * thetaWPrime / (4.0 * thetaFWHM))));
    ...
    coef = determineModelCoef(modelType, count, inRange, checks, alpha,
                              scans[i_0].getDec(j_0), scans[i_0].getRa(j_0));
    ...
    for (int i = 0; i < inRange.size() / 6; i++) {
      ...
      zPrime = applySurfaceModel(
          xPrime, yPrime, inRange[i * 6],
          coef); //	WE NEED TO CHANGE MODELS IF IT GIVES INFINITY
      ...
      secondDeriv = calculateSecondDerivitive(
          modelType, iTemp, jTemp, thetaPerp,
          coef); // possibly not being calculated for m6
      ...
      // if ((signedDelta > 0 && secondDeriv < 0) || (signedDelta < 0 &&
      // secondDeriv > 0))
      //{
      //	if (wVarience > max)
      //	{
    ...
    if (count > 1 && maxI != -1) {
      if (i_0 == 59 && j_0 == 10) {
        ...
        SurPoints << sigma << "\t" << avgScatter << "\t"
                  << inRange[6 * maxI + 4] << "\t" << inRange[6 * maxI + 5]
                  << "\t"
                  << "rejected\n";
        SurPoints.close();
      }
...
double Processor::lssRecursivePolyFit(Survey &survey, int i_0, int j_0,
                                      double bgScale, double wScale) {
  ...
  double preScanDist =
      Tools::getGCDistance(scans[i_0].getDec(j_0), scans[i_0].getRa(j_0),
                           scans[i_0].getDec(0), scans[i_0].getRa(0),
                           partSetProcSSS.centerDecDeg) *
      toDeg;
  double proScanDist =
      Tools::getGCDistance(scans[i_0].getDec(j_0), scans[i_0].getRa(j_0),
                           scans[i_0].getDec(scans[i_0].getSize() - 1),
                           scans[i_0].getRa(scans[i_0].getSize() - 1),
                           partSetProcSSS.centerDecDeg) *
      toDeg;
  double thetaBG = bgScale; // UNITS OF BEAMS
  double thetaEnd =
      std::max(proScanDist, preScanDist) / psfFWHM; // UNITS OF BEAMS
  double thetaFWHM = std::max(std::min(thetaBG / 3.0, 2.0 * thetaEnd / 3.0),
                              1.0); // UNITS OF BEAMS

  double largestGap = scans[i_0].getLSSThetaGap(j_0);
  double thetaMinPrime = wScale * thetaFWHM; // 2 * thetaFWHM / 3.0;
  double thetaWPrime = Tools::max(
      thetaMinPrime,
      Tools::min(((4.0 / 3.0) * largestGap * toDeg / psfFWHM), thetaFWHM));
  double alpha = -1.0 * std::log(2) /
                 (std::log(std::cos(M_PI * thetaWPrime / (4.0 * thetaFWHM))));

  std::vector<double> LSS2DScatter = survey.getLSS2DScatter();

  pointCandidates = findPossLSS(scans[i_0].getDec(j_0), scans[i_0].getRa(j_0),
                                thetaFWHM * psfFWHM, possibles);
  inRange.reserve(pointCandidates *
                  6); // THIS MAY BE CHANGED TO RESERVE INSTEAD OF RESIZE
  ...
  for (int k = 0; k < pointCandidates; k++) {
    ...
    distance = Tools::getGCDistance(
                   scans[i_0].getDec(j_0), scans[i_0].getRa(j_0),
                   scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
                   partSetProcSSS.centerDecDeg) *
               toDeg;
  ...
  rejectModelPoints(i_0, j_0, thetaMinPrime, thetaFWHM, avgScatter, inRange,
                    checks);
  ...
  std::vector<double> results = LSSCollectResults(
      i_0, j_0, thetaFWHM, LSS2DScatter, coef, checks, inRange);
...
double Processor::applySurfaceModel(double xPrime, double yPrime, double zOrig,
                                    std::vector<double> &coef) {
  double zPrime;
  switch (coef.size()) {
  case 10:
    zPrime = coef[0] + coef[1] * xPrime + coef[2] * yPrime +
             coef[3] * xPrime * xPrime + coef[4] * yPrime * yPrime +
             coef[5] * xPrime * yPrime + coef[6] * xPrime * xPrime * xPrime +
             coef[7] * yPrime * yPrime * yPrime +
             coef[8] * xPrime * xPrime * yPrime +
             coef[9] * xPrime * yPrime * yPrime;
    break;
  case 6:
    zPrime = coef[0] + coef[1] * xPrime + coef[2] * yPrime +
             coef[3] * xPrime * xPrime + coef[4] * yPrime * yPrime +
             coef[5] * xPrime * yPrime;
    break;
  case 3:
    zPrime = coef[0] + coef[1] * xPrime + coef[2] * yPrime;
    break;
...
double Processor::calculateSecondDerivitive(SurfaceType modelType, int i_0,
                                            int j_0, double theta,
                                            std::vector<double> &coef) {
  int Ni, Nj;
  int iTemp, jTemp;
  int count = 0;
  ...
  for (int i = 0; i <= Ni; i++) {
    for (int j = 0; j <= i; j++) {
      secondDerivative += coef[count] *
                          ((i * (i - 1)) * pow(x, (i - 2)) * pow(y, j) *
                               (1 / pow(std::cos(theta), 2)) +
                           2 * i * j * pow(x, i - 1) * pow(y, j - 1) *
                               (1 / std::cos(theta)) * (1 / std::sin(theta)) +
                           j * (j - 1) * pow(x, i) * pow(y, j - 2) *
                               (1 / pow(std::sin(theta), 2))) *
                          (pow(std::cos(theta), 2) * pow(std::sin(theta), 2));
      count++;
    }
  }
...
Processor::calculateScanNumbers(int &count, std::vector<double> &inRange,
                                std::vector<bool> &checks) {
  int holdI, holdJ;
  bool contained;
  std::vector<int> intFiller;
...
std::vector<double> Processor::LSSCollectResults(
    int i_0, int j_0, double thetaFWHM, std::vector<double> &LSS2DScatter,
    std::vector<double> &coef, std::vector<bool> &checks,
    std::vector<double> &inRange) {
  double x0 = scans[i_0].getDec(j_0);
  double y0 = scans[i_0].getRa(j_0);
  double centerLSSFlux =
      applySurfaceModel(0, 0, scans[i_0].getLSSData(j_0), coef);
  ...
  for (int i = 0; i < inRange.size() / 6; i++) {
    ...
    if (checks[i]) {
      ...
      for (int j = 0; j < inRange.size() / 6; j++) {
        ...
        if (wCorrMap) // We determine the first correction factor and the
                      // correlation scale for photometry here.
        {
          rfiCountHold = 0.0;
          countAbove =
              (centerLSSFlux > sqrt(2.0) * correlatedScatter) ? true : false;
          fluxAbove = (jLSSFlux > sqrt(2.0) * correlatedScatter) ? true : false;

          dd = scans[jHoldI].getDataDumps(jHoldJ);
          pointDistance = Tools::getGCDistance(iDec, iRa, jDec, jRa,
                                               partSetProcSSS.centerDecDeg) *
                          toDeg; // CHECK THAT fwhmRfi*toDeg is right units

          if (checks[j] && (pointDistance < thetaFWHM * psfFWHM) &&
              ((countAbove && fluxAbove) ||
               (!countAbove &&
                !fluxAbove))) { // Includes all points that are within a
                                // rfi-beam of LMV && within a rfi-beam of GMV
                                // && are (Case 1: above OR Case 2: below) the
                                // 2d-scatter.
            rfiCountHold += 1;
            if ((pointDistance != 0)) {
              wPointCount += 1.0 * dd;
      ...
      if (inRange.size() / 6 < 3) {
        ...
        LMWeight = (dwSum / (1.0 + 2.0 * pow((pw - pwAvg), 2) /
                                       (pwDelta / pwDeltaCounter)));
      }
      ...
      results.push_back(
          rfiCountHold); // number of points that satisfy conditions (1), (2),
                         // and (3) in appendix D. (Used to divide local model
                         // weights that go into the global model weight)
...
SurfaceType
Processor::determineSurfaceType(std::vector<std::vector<int>> &scanNumbers,
                                MapTypes mapType) {
  SurfaceType modelType;
...
std::vector<double> Processor::determineModelCoef(
    SurfaceType modelType, int pointCount, std::vector<double> &inRange,
    std::vector<bool> &checks, double alpha, double dec, double ra) {
  std::vector<double> coef;
  switch (modelType) {
  case M10:
...
std::vector<double> Processor::surfaceFit3(std::vector<double> &inRange,
                                           std::vector<bool> &checks,
                                           double alpha, double i_0,
                                           double j_0) {
  ...
  double w = 0, xw = 0, yw = 0, xxw = 0, yyw = 0, xyw = 0, xxxw = 0, xyyw = 0,
         xxyw = 0, yyyw = 0, xxxxw = 0, xxyyw = 0, xxxyw = 0, yyyyw = 0,
         xyyyw = 0, zw = 0, xzw = 0, yzw = 0, xxzw = 0, yyzw = 0, xyzw = 0,
         xHold, yHold, zHold, wHold, wLocalW = 0, wLocalW1 = 0, wLocalWInv = 0,
         finalAnswer;
...
std::vector<double> Processor::surfaceFit6(std::vector<double> &inRange,
                                           std::vector<bool> &checks,
                                           double alpha, double i_0,
                                           double j_0) {
  ...
  // double alpha = (log(0.5)) / (log(cos(M_PI*thetaWPrime / (4.0
  // *thetaFWHM))));
  double withinRfiDec, withinBeamDec, withinRfiRa, withinBeamRa;
  int iTemp, jTemp;
...
std::vector<double> Processor::surfaceFit10(std::vector<double> &inRange,
                                            std::vector<bool> &checks,
                                            double alpha, double i_0,
                                            double j_0) {
  ...
  double w = 0, xw = 0, yw = 0, xxw = 0, yyw = 0, xyw = 0, xxxw = 0, xyyw = 0,
         xxyw = 0, yyyw = 0, xxxxw = 0, xxyyw = 0, xxxyw = 0, yyyyw = 0,
         xyyyw = 0, xxxxxw = 0, xxyyyw = 0, xxxxyw = 0, xxxyyw = 0, yyyyyw = 0,
         xyyyyw = 0, xxxxxxw = 0, xxxxxyw = 0, xxxxyyw = 0, xxxyyyw = 0,
         xxyyyyw = 0, xyyyyyw = 0, yyyyyyw = 0, zw = 0, xzw = 0, yzw = 0,
         xxzw = 0, yyzw = 0, xyzw = 0, xxxzw = 0, yyyzw = 0, xxyzw = 0,
         xyyzw = 0, xHold, yHold, zHold, wHold, finalAnswer;
  std::vector<double> A;
  std::vector<double> b;
  std::vector<double> coef;
  std::vector<int> possibles;
...
void Processor::levelLSSData(std::vector<Survey> &surveys) {
    ...
    for (int j = 0; j < surveys.size(); j++) {
      ...
      for (int k = 0; k < scansI.size(); k++) {
        for (int l = 0; l < scansI[k].getSize(); l++) {
          ...
          inBounds = checkEdgeCriteria(surveys[i].getProcessingCoordinate(),
                                       partSetJ, ra, dec);
          if (inBounds) {
            zPrime =
                surfaceModel(scansJ, partSetJ, ra, dec, largestGap, wScaleBW) -
                scansI[k].getLSSData(l);
  ...
  while (maxDiff > 0.01) {
    devDiff = 0;
    for (int i = 1; i < surveys.size();
         i++) // always let one of the deviations be zero
    {
      devOld = deviations[i];
      num = 0;
      denom = 0;
...
void Processor::subtractDeviations(std::vector<Survey> &surveys,
                                   std::vector<double> &deviations) {
...
double Processor::surfaceModel(std::vector<Scan> scansJ, PartitionSet partSetJ,
                               double ra, double dec, double largestGap,
                               double wScale) {
  ...
  inRange.resize(pointCandidates *
                 6); // THIS MAY BE CHANGED TO RESERVE INSTEAD OF RESIZE
  checks.resize(pointCandidates, true);
  ...
  for (int k = 0; k < pointCandidates; k++) {
    ...
    distance = Tools::getGCDistance(dec, ra, scansJ[holdI].getDec(holdJ),
                                    scansJ[holdI].getRa(holdJ),
                                    partSetProcSSS.centerDecDeg) *
               toDeg;
    ...
    if (distance < psfFWHM) {
      ...
      inRange[count * 6 + 2] = 999999; // THESE ARE SET SO THAT THE INDEXING OF
                                       // THE SURFACE FIT FUNCTION STILL WORKS;
  ...
  scale = Tools::max(
      wScale, Tools::min((4.0 / 3.0) * largestGap / (psfFWHM * toRad), 1.0));

  alpha =
      -1.0 * std::log(2) / (std::log(std::cos(M_PI * scale / (4.0 * psfFWHM))));
...
bool Processor::checkEdgeCriteria(Coordinates coordinate,
                                  PartitionSet partSetProcSSS, double ra,
                                  double dec) {
  bool withinBounds = false;
  MapTypes mapType;

  std::vector<double> coordinateCheck, edgeOneParameters, edgeTwoParameters,
      edgeThreeParameters, edgeFourParameters;
  mapType = partSetProcSSS.mapType;
  bool tracking = partSetProcSSS.tracking;

  coordinateCheck = determineMappedEdges(ra, dec, tracking, partSetProcSSS,
                                         coordinate, mapType);
...
std::vector<double>
Processor::determineMappedEdges(double LONG, double LATI, bool tracking,
                                PartitionSet compPartSetProc,
                                Coordinates coordinate, MapTypes mapType) {
  double centerProcLati, centerProcLong, centerMapLati, centerMapLong;
  double unTransLati, unTransLong, unTransLatiVert, unTransLongVert, doAngDeg,
      undoAngDeg;
  ...
  if (!(mCoordinate == "galactic" && pCoordinate == GALACTIC) &&
      !(mCoordinate == "equatorial" &&
        pCoordinate == EQUATORIAL)) // if mapping coords != processing coords.
  {
    if (pCoordinate == GALACTIC) { // This is in limbo because Skynet is wrong.
      if (tracking) {
        centerProcLati = Tools::convertToB(compPartSetProc.medianLongMap,
                                           compPartSetProc.medianLatiMap);
        centerProcLong = Tools::convertToL(compPartSetProc.medianLongMap,
                                           compPartSetProc.medianLatiMap);
      ...
      if (tracking) {
        centerProcLati = Tools::convertToDec(compPartSetProc.medianLongMap,
                                             compPartSetProc.medianLatiMap);
        centerProcLong = Tools::convertToRa(compPartSetProc.medianLongMap,
                                            compPartSetProc.medianLatiMap);
        doAngDeg = compPartSetProc.medianLatiMap;
        undoAngDeg = centerProcLati;
      } else {
    ...
    unTransLati = LATI + centerProcLati; // these two lines do the un-transform
                                         // in the processing coords.
    ...
    if (mapType == NODDING) {
      ...
      LATI =
          unTransLatiVert -
          centerMapLati; // these two lines re-transform in the mapping coords.
      LONG = (unTransLongVert - centerMapLong) * cos(doAngDeg * toRad);
    }
  } else // if mapping coords == processing coords.
  {
    if (mapType == NODDING) // edges for NODDINGs are now checked in
                            // un-transformed coordinates.
    {
...
std::vector<double> Processor::autoFixedWRegression(int start, int end,
std::vector<bool> checks, std::vector<double> weights, std::vector<double>
horizAxis, std::vector<double> vertAxis, double horizAxisCoordinate, double
vertAxisCoordinate)
{
...
quadCoef = Tools::fixedQuadraticRegression(start, end, checks, weights,
horizAxis, vertAxis, horizAxisCoordinate, vertAxisCoordinate); linCoef =
Tools::fixedRegression(start, end, checks, weights, horizAxis, vertAxis,
horizAxisCoordinate, vertAxisCoordinate);
...
std::vector<double> Processor::autoWRegression(int start, int end,
std::vector<bool> checks, std::vector<double> weights, std::vector<double>
horizAxis, std::vector<double> vertAxis)
{
//ATTEMPTS QUADRATIC REGRESSION
//IF A COEFFICENT IS UNSTABLE RESORST TO LINEAR
//IF LINEAR FALIS NO MODEL
...
quadCoef = Tools::quadraticRegressionPivot(start, end, checks, weights,
horizAxis, vertAxis); linCoef = Tools::regressionPivot(start, end, checks,
weights, horizAxis, vertAxis);
...
std::vector<double> Processor::applyModel(double x, double x0,
...
void Processor::calculateBaselines(bool forward, int start, int end, int size,
double scatter, std::vector<double> &angDist, std::vector<double> &flux,
std::vector<double> &dumps, std::vector<std::vector<double> > &bgData,
std::vector<std::vector<double> > &bgWeights, std::vector<double> &ra,
...
double largest, smallest, splus = 999999, holderA, holderB,
regressionHorizValue, regressionVertValue, splusTemp = 0, splusSigmaDiff =
999999; double distinctValueHold; double distinctYHold; bool isLinear = false;
bool testBool;
int counter = 0, counterplus, largeIndex, low = 999999, high = -999999,
smallIndex, offset, lastChecked; int checkCount; std::ofstream testFile;
...
//THIS MEANS WE MUST RESTRICT THE QUADRATIC OR LINEAR MODEL TO FIT THROUGH THE
regression VALUES if (forward)
{
checks[start] = false;
...
checkCount = distinctXValues.size();
regressionLine = autoFixedWRegression(0, end - start, checksTemp, dumpsTemp,
angDistTemp, zTemp, regressionHorizValue, regressionVertValue);
...
//REBUILD MODELS
regressionLine = autoWRegression(0, end - start, checksTemp, dumpsTemp,
angDistTemp, zTemp);
...
for (int k = std::max(low - 1, start + offset); k < std::min(high + 2, end +
offset); k++)
{
...
if (holderBVec[0] == NAN || holderBVec[0] != holderBVec[0] || stDev == 0 ||
kurtosis == 0)
{
...
{
bgWeights[k].push_back(dumpSum / (1 + pow((angDist[k] - mean) / stDev, 2) +
pow((angDist[k] - mean) / kurtosis, 4)));
}
}
}
}
...
//FIND THE MINIMUM AND MAXIMUM INDEX VALUES FOR A BASELINE LENGTH STARTING AT
INDEX start for (; start < size - 2; start++)
{
...
if (end != start)
{
calculateBaselines(true, start, end, size, scatter, angDist, flux, dumps,
bgData, bgWeights, ra, dec);
}
...
if (start != end)
{
calculateBaselines(false, start, end, size, scatter, angDist, flux, dumps,
bgData, bgWeights, ra, dec);
}
}
...
for (int j = 0; j < bgData[i].size(); j++)
{
if (bgData[i][j] == NAN || bgData[i][j] != bgData[i][j] || bgWeights[i][j] ==
NAN || bgWeights[i][j] != bgWeights[i][j])
{
trueCount--;
}
else
...
&Processor::calculateBGMulti, this, i, bgScale*psfFWHM);
                }
                for (int i = 0; i < scans.size(); i++)
                {
...
// OLD FUNCTIONS
// std::vector<double> rfiFit(int pointsInRange, double avgSigma,
// std::vector<double> &sigmaValues, std::vector<int> &checks,
// std::vector<double> &inRange)
//{
//	double sigmaTerm = 0, fluxShapeTerm = 0, shapeSquaredTerm = 0, shapeTerm
//= 0, fluxTerm = 0; 	double f, g, sigma, sigmaSquare, flux, shape, dump;
...
//			shapeSquaredTerm += (dump * shape * shape) /
// sigmaSquare; 			shapeTerm += dump * shape / sigmaSquare;
// fluxTerm += dump * flux
/// sigmaSquare;
//		}
//	}
//	f = (2 * sigmaTerm * fluxShapeTerm - fluxTerm * shapeTerm) / (2 *
// sigmaTerm * shapeSquaredTerm - shapeTerm * shapeTerm); 	g = (fluxTerm -
// f * shapeTerm) / (2 * sigmaTerm); 	solution[0] = f; 	solution[1] = g;
// return solution;
// }
// void Processor::rfiRemovePoints(int pointsInRange, double avgSigma,
// std::vector<int> & holdIVec2, std::vector<int> & holdJVec2,
// std::vector<double> &sigmaValues, std::vector<int> &checks,
// std::vector<double> &inRange)
//{
...
//		counterplus = 0;
//		largeIndexI = -1;
//		rfiFitSolution = rfiFit(pointsInRange, avgSigma, sigmaValues,
// checks, inRange); 		f = rfiFitSolution[0]; 		g =
// rfiFitSolution[1]; 		if (f > 0)
//		{
//			for (int i = 0; i < pointsInRange; i++)
//			{
//				if (checks[i])
...
//					if (shape > modelHold)
//					{
//						splus = splus + pow(shape -
// modelHold, 2); 						counterplus++;
//					}
...
//					modelHold = f * weight + g;
//					counter++;
//					splus = splus + pow(shape - modelHold,
// 2); 					counterplus++;
...
//				scans[holdIVec2[largeIndexI]].setLocalModelRejections(holdJVec2[largeIndexI],
// scans[holdIVec2[largeIndexI]].getLocalModelRejections(holdJVec2[largeIndexI])
//+ 1); 				lastChecked = largeIndexI;
...
//	int pointsInRange = 0, normalLocalWeightCounter = 0, counter,
// counterplus, largeIndexI, holdI, holdJ, pointCandidates = 0,
// pointsNotRejected = 0, pointsInLasso = 0, lastChecked, rfiCountHold = 0;
// double avgSigma = 0, largestDiff = 0, splus = 999999, fitHold = 0, largest,
// f,
// g, shape, weight, distance, angleHold, splusSigmaDiff = 999999, toRad = M_PI
// / 180.0; 	double raHold = scans[i_0].getRa(j_0), weightHold,
// localModelDumpSum, localModelShapeSum; 	double shapeParam,
// averageInRangeDec = 0, modelHold, normalLocalWeight = 0, distanceTemp,
// shapeParamTemp, wResult; 	std::vector<int> checks, holdIVec, holdJVec,
// holdIVec2, holdJVec2; 	std::vector<double> dubFiller, distances,
// inRange, ra, dec, highs, lows, sigmaValues, rfiFitSolution, results;
//
...
//
//	pointCandidates = findPossSSS(scans[i_0].getDec(j_0),
// scans[i_0].getRa(j_0), fwhmRfi*toDeg, possibles);
...
//
//	//COLLECT POSSIBLES AND STORE INFORMATION ABOUT PROXIMITY, SCATTER, AND
// FLUX 	for (int i = 0; i < pointCandidates; i++)
//	{
//		holdI = possibles[2 * i];
//		holdJ = possibles[2 * i + 1];
//
//		distance = Tools::getGCDistance(scans[i_0].getDec(j_0),
// scans[i_0].getRa(j_0), scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// partSetProcSSS.centerDecDeg); 		distances[i] = distance;
// if (distance < fwhmRfi)
//		{
//			scans[holdI].setLocalModelInstances(holdJ,
// scans[holdI].getLocalModelInstances(holdJ) + 1);
// holdIVec2.push_back(holdI); 			holdJVec2.push_back(holdJ);
//			inRange.push_back(scans[holdI].getFlux(holdJ));
//			inRange.push_back(pow(cos((M_PI*distance) /
//(fwhmRfi*2.0)), 2));
// inRange.push_back(scans[holdI].getDataDumps(holdJ));
//			sigmaValues.push_back(scatter2d[scans[holdI].getSurveyNumber()][scans[holdI].getScanNumberInSurvey()]);
//			pointsInRange++;
//			avgSigma +=
// scatter2d[scans[holdI].getSurveyNumber()][scans[holdI].getScanNumberInSurvey()];
//		}
//	}
//
...
//
//
//	//FIT A LOCAL MODEL AND REJECT MOST DEVIANT POINT UNTIL WITHIN TOLERATED
// SCATTER
//
//
//	/*while (splus > avgSigma)
...
//	}*/
//
//	rfiRemovePoints(pointsInRange, avgSigma, holdIVec2, holdJVec2,
// sigmaValues, checks, inRange);
//
//	//FIT FINAL LOCAL MODEL
//	rfiFitSolution = rfiFit(pointsInRange, avgSigma, sigmaValues, checks,
// inRange); 	f = rfiFitSolution[0]; 	g = rfiFitSolution[1];
//
...
// True;
//
//	//ITERATE THROUGH ALL ORIGINAL COLLECTED POINTS AND STORE THEIR RA, DEC,
// SCANINDEX, DATA INDEX 	int trueCount = 0;
// holdIVec.reserve(distances.size()); 	holdJVec.reserve(distances.size());
// for
//(int i = 0; i < pointCandidates; i++)
//	{
...
//			if (trueCount != i)
//			{
//				distances[trueCount] = distances[i];//ONLY STORE
// DISTANCES OF POINTS WITHIN RFI RADIUS, NOT OF ALL COLLECTED POSSIBLES
//			}
//			trueCount++;
//		}
//	}
...
//			shapeParam = pow(cos((M_PI*distance) / (fwhmRfi*2.0)),
// 2); 			fitHold = f * shapeParam + g;
//
//			if (correlatedWeightMap)
//			{
//				//We determine the first correction factor and
// the correlation scale for photometry here.
// rfiCountHold = 0.0; 				countAbove = false, countBelow =
// false, rejectPoints = true; 				correlatedScatter =
// scatter2d[scans[i_0].getSurveyNumber()][scans[i_0].getScanNumberInSurvey()];
//
//				if (scans[i_0].getFlux(j_0) >
// sqrt(2.0)*correlatedScatter) { countAbove = true;
//				}
//				else {
//					countBelow = true;
//				}
...
//
//					rfiDistance =
// Tools::getGCDistance(scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// scans[jHoldI].getDec(jHoldJ), scans[jHoldI].getRa(jHoldJ),
// partSetProcSSS.centerDecDeg)*toDeg; 					if
// (rfiDistance < fwhmRfi*toDeg)
//					{
//						if (countAbove &&
// scans[jHoldI].getFlux(jHoldJ) > sqrt(2.0)*correlatedScatter) {
// rfiCountHold += 1;	//Includes all points that are within a rfi-beam of LMV
// && within a rfi-beam of GMV && are above the 2d-scatter.
//						}
//						if (countBelow &&
// scans[jHoldI].getFlux(jHoldJ) < sqrt(2.0)*correlatedScatter) {
// rfiCountHold += 1; //Includes all points that are within a rfi-beam of LMV &&
// within a rfi-beam of GMV && are below the 2d-scatter.
//						}
//					}
...
// Tools::getGCDistance(scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// scans[jHoldI].getDec(jHoldJ), scans[jHoldI].getRa(jHoldJ),
// partSetProcSSS.centerDecDeg)*toDeg;
...
//					shapeParamTemp =
// pow(cos((M_PI*distanceTemp) / (fwhmRfi*2.0)), 2);
// normalLocalWeight += pow(shapeParamTemp - localModelShapeSum, 2); //sum of
// proximity weight of this point minus all other points used to make the local
// model squared. normalLocalWeightCounter++;
//				}
//			}
//			else
//			{
...
//			}
//			else {
//				wResult = (localModelDumpSum / (1.0 + 2.0 *
// pow((shapeParam - localModelShapeSum), 2) / (normalLocalWeight /
...
//	return results;
// }
// void Processor::extraLocalModels(double decPoint, double raPoint,
// std::vector<std::vector<std::vector<double> > > &rfiValues,
// std::vector<std::vector<std::vector<double> > > &rfiWeights,
// std::vector<std::vector<std::vector<double> > >&rfiCounts,
// std::vector<std::vector<std::vector<double> > > & rfiDists,
// std::vector<std::vector<std::vector<double> > > & rfiNs, double rfiScale,
// std::vector<MapTypes> mapHolder)
//{
//	int rfiCountHold = 0;
//	double comDec, comRa;
...
//			{
//				std::vector<int> possibles;
//				bool nonRejectedPointInTestScan = false,
// writeModel = false; 				int pointsInRange = 0, counter,
// normalLocalWeightCounter = 0, counterplus, largeIndexI, holdI, holdJ,
// pointCandidates = 0, pointsNotRejected = 0, pointsInLasso = 0, lastChecked;
// double avgSigma = 0, largestDiff = 0, splus = 999999, fitHold = 0, largest,
// f, g, shape, weight, distance, angleHold, splusSigmaDiff = 999999, toRad =
// M_PI / 180.0; 				double weightHold,
// localModelDumpSum, localModelShapeSum; 				double
// shapeParam, averageInRangeDec = 0, modelHold, normalLocalWeight = 0,
// distanceTemp, shapeParamTemp; 				std::vector<int>
// checks, holdIVec, holdJVec; 				std::vector<double>
// dubFiller, distances, inRange, ra, dec, highs, lows, sigmaValues,
// rfiFitSolution;
//
//				double deltaX = i * (standardGap)*(pow((1 /
// rfiScale), 2) - 1); //In degrees 				double deltaY =
// j * (standardGap)*(pow((1 / rfiScale), 2) - 1);
//
//				double sampleWidth = psfFWHM * (1 -
// pow(rfiScale, 2));
//
//				distance = Tools::getGCDistance((decPoint +
// deltaX), (raPoint + deltaY), decPoint, raPoint,
// partSetProcSSS.centerDecDeg)*toDeg;
//
...
//				}
//
//				pointCandidates = findPossSSS((decPoint +
// deltaX), (raPoint + deltaY), fwhmRfi*toDeg, possibles);
//
//				distances.resize(pointCandidates, 0.0);
//				inRange.reserve(pointCandidates * 2);
//				sigmaValues.reserve(pointCandidates);
...
//					holdJ = possibles[2 * i + 1];
//					distance =
// Tools::getGCDistance((decPoint + deltaX), (raPoint + deltaY),
// scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// partSetProcSSS.centerDecDeg);
//
...
//					counterplus = 0;
//					largeIndexI = -1;
//					rfiFitSolution = rfiFit(pointsInRange,
// avgSigma, sigmaValues, checks, inRange);
// f = rfiFitSolution[0]; 					g =
// rfiFitSolution[1]; 					if (f > 0)
//					{
//						for (int i = 0; i <
// pointsInRange; i++)
...
//
//								splus = splus +
// pow(shape - modelHold, 2);
// counterplus++;
//
//								if
//(std::abs(shape - modelHold) > largest)
//								{
//									largest
//= shape - modelHold;
// largeIndexI = i;
...
//					{
//						splus = sqrt((splus /
//(counterplus))); 						if ((splus >
// avgSigma) && (largeIndexI > -1))
//						{
//							checks[largeIndexI] =
// false; 							lastChecked =
// largeIndexI;
...
//					}
//				}
//				rfiFitSolution = rfiFit(pointsInRange, avgSigma,
// sigmaValues, checks, inRange);
// localModelDumpSum = 0; 				localModelShapeSum = 0;
//				int checksTrue = 0;
//				for (int i = 0; i < checks.size(); i++)
//				{
//					if (checks[i])
//					{
//						localModelDumpSum += inRange[3 *
// i + 2];
//						//localModelShapeSum +=
// pow(inRange[3 * i + 1], 2);
// localModelShapeSum += inRange[3 * i + 1];
...
//					}
//				}
//				//localModelShapeSum = localModelShapeSum /
//(double)checks.size(); 				localModelShapeSum =
// localModelShapeSum / (double)checksTrue; 				f =
// rfiFitSolution[0]; 				g = rfiFitSolution[1];
// int trueCount = 0; 				ra.reserve(distances.size());
...
//					}
//				}
//				distances.erase(distances.begin() + trueCount,
// distances.end());
//
//				for (int i = 0; i < holdIVec.size(); i++)
//				{
...
//						distance = distances[i];
//						shapeParam =
// pow(cos((M_PI*distance) / (fwhmRfi*2.0)), 2);
// fitHold = f * shapeParam + g;
//
//						if (correlatedWeightMap)
//						{
//							//We determine the first
// correction factor and the correlation scale for photometry here.
// rejectPoints = true;
// correlatedScatter =
// scatter2d[scans[holdI].getSurveyNumber()][scans[holdI].getScanNumberInSurvey()];
//
//							//We assume that each
// grid point is above sqrt(2)*scatter.
//
//							for (int j = 0; j <
// holdIVec.size(); j++)
//							{
//								rfiDistance =
// Tools::getGCDistance(scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// scans[holdIVec[j]].getDec(holdJVec[j]),
// scans[holdIVec[j]].getRa(holdJVec[j]), partSetProcSSS.centerDecDeg)*toDeg;
// if (rfiDistance < fwhmRfi*toDeg)
//								{
//									if
//(scans[holdIVec[j]].getFlux(holdJVec[j]) > sqrt(2.0)*correlatedScatter) {
//										//Includes
// all points that are within a rfi-beam of LMV && within a rfi-beam of GMV &&
// are above the 2d-scatter.
// rfiCountHold += 1;
//									}
//								}
//								if (checks[j])
////Correlation scale here.
//								{
//									pointDistance
//= Tools::getGCDistance(scans[holdI].getDec(holdJ), scans[holdI].getRa(holdJ),
// scans[holdIVec[j]].getDec(holdJVec[j]),
...
//
//											wPointCount
//+= 1.0*scans[holdIVec[j]].getDataDumps(holdJVec[j]);
// wDistSq += pow(pointDistance,
// 2)*scans[holdIVec[j]].getDataDumps(holdJVec[j]);
//										}
//									}
//
//								}
...
//						}
//						else {
//							rfiWeights[holdI][holdJ].push_back((localModelDumpSum
/// (1.0 + 2.0 * pow((shapeParam - localModelShapeSum), 2) / (normalLocalWeight
/// normalLocalWeightCounter))));
//						}
//
...
/// normalLocalWeightCounter))));
//						//rfiWeights[holdI][holdJ].push_back((localModelDumpSum
/// (1.0 + 2.0 * (shapeParam*shapeParam) / (localModelShapeSum))));
...
//				double avgSigma = 0, distance, toRad = M_PI /
// 180.0; 				double correlatedScatter, rfiDistance,
// pointDistance, fluxVal; 				std::vector<int> checks,
// possibles;
//				std::vector<double> inRange, rfiFitSolution,
// results;
//
//				double deltaX = i * (standardGap)*(pow((1 /
// rfiScale), 2) - 1); //In degrees 				double deltaY =
...
//				results = rfiCollectResults(999999, -999999,
// rfiFitSolution, checks, inRange);//We assume these data are above the
// correlated scatter value
//			}
//		}
//	}
// }
...
/*
void Processor::autoCentroid(std::vector<std::vector<std::vector<double> > >
rfiValues)
{
...
for (int j = 0; j < scans[i].getSize(); j++)
{
if ((scans[i].getFlux(j) < (75 * scans[i].getScatter())) ||
(rfiValues[i][j].size() == 0))
{
sourceFlag[j] = false;
}
else
...
//CALCULATES THE WEIGHTED CENTER OF MASS OF FLAGGED FLUXES WITHIN A ONE BEAM
WIDTH RADIUS bool newSource = false; int iHold, jHold, iTemp, jTemp; int
counter, chs, pointCandidates; double wDec, wRa; double comDec, comRa; double
maxFlux, totalFlux; double distance; double centroidDistance; double
potentialRa, potentialDec; std::vector<double> coordinates, inRange;
...
distance = Tools::getGCDistance(scans[iHold].getDec(jHold),
scans[iHold].getRa(jHold), scans[iHold].getDec(jHold),
scans[iHold].getRa(jHold), partSetProcSSS.centerDecDeg)*toDeg;
...
pointCandidates = findPossSSS(scans[iHold].getDec(jHold),
scans[iHold].getRa(jHold), psfFWHM, possibles);
...
distance = Tools::getGCDistance(scans[iTemp].getDec(jTemp),
scans[iTemp].getRa(jTemp), scans[iHold].getDec(jHold),
scans[iHold].getRa(jHold), partSetProcSSS.centerDecDeg)*toDeg;
...
coordinates = determineCenters(counter, inRange,
scans[iHold].getThetaGap(jHold), scans[iHold].getDec(jHold),
scans[iHold].getRa(jHold));
...
{
centroidDistance = Tools::getGCDistance(centroidLocations[2 * i],
centroidLocations[2 * i + 1], potentialDec, potentialRa,
partSetProcSSS.centerDecDeg)*toDeg; if (centroidDistance > psfFWHM  && counter
>= 6)
{
newSource = true;
}
...
std::vector<double> Processor::determineCenters(int pointCount,
std::vector<double>& inRange, double largestGap, double i_0, double j_0)
{
...
weightingFunctionTemp = (1.0 / 3.0);//Tools::max(weightingFunction,
Tools::min((4.0 / 3.0)*largestGap*toDeg / psfFWHM, 1.0));//maybe
weightTempNeeded double alpha = (log(0.5)) / (log(cos(M_PI*weightingFunctionTemp
/ 4.0)));
...
A[18] = xxw; A[19] = xxxw; A[20] = xxyw; A[21] = xxxxw; A[22] = xxyyw; A[23] =
xxxyw; A[24] = yyw; A[25] = xyyw; A[26] = yyyw; A[27] = xxyyw; A[28] = yyyyw;
A[29] = xyyyw; A[30] = xyw; A[31] = xxyw; A[32] = xyyw; A[33] = xxxyw; A[34] =
xyyyw; A[35] = xxyyw; b[0] = zw; b[1] = xzw; b[2] = yzw; b[3] = xxzw; b[4] =
yyzw; b[5] = xyzw;

coef = Tools::matrixSolver(columnCount, A, b);

double x = ((coef[2] * coef[5]) - (2 * coef[1] * coef[4])) / ((4 * coef[3] *
coef[4]) - pow(coef[5], 2)); double y = ((coef[1] * coef[5]) - (2 * coef[2] *
coef[3])) / ((4 * coef[3] * coef[4]) - pow(coef[5], 2));
...
*/
/*
std::vector<double> rfiFit(std::vector<int> &checks, std::vector<double>
&inRange)
{
double sigmaTerm = 0, fluxShapeTerm = 0, shapeSquaredTerm = 0, shapeTerm = 0,
fluxTerm = 0; double f, g, sigma, sigmaSquare, flux, shape, dump;
std::vector<double> solution;
solution.resize(2);
for (int i = 0; i < inRange.size() / 6; i++)
{
...
f = (2 * sigmaTerm * fluxShapeTerm - fluxTerm * shapeTerm) / (2 * sigmaTerm *
shapeSquaredTerm - shapeTerm * shapeTerm); g = (fluxTerm - f * shapeTerm) / (2 *
sigmaTerm); solution[0] = f; solution[1] = g; return solution;
}
void Processor::rfiRemovePoints(double avgSigma, std::vector<int> &checks,
std::vector<double> &inRange)
{
...
void Processor::rfiBuildLocalModels(std::vector<std::vector<std::vector<double>
> >& rfiWeights, std::vector<std::vector<std::vector<double> > >& rfiValues,
std::vector<std::vector<std::vector<double> > >& rfiCounts,
std::vector<std::vector<std::vector<double> > > & rfiDists,
std::vector<std::vector<std::vector<double> > > &rfiNs)
{
...
{
correlatedScatter =
scatter2dSSS[scans[i].getSurveyNumber()][scans[i].getScanNumberInSurvey()];
...
futureFiller[j] = std::async(std::launch::async, &Processor::rfiRemovalMulti,
this, decHold, raHold, fluxVal, correlatedScatter);
}

rfiSetFinalValues(futureFiller, rfiWeights, rfiValues, rfiCounts, rfiDists,
rfiNs);
}
}
void Processor::rfiBuildExtraModels(double decPoint, double raPoint, double
standardGap, std::vector<std::vector<std::vector<double> > >& rfiWeights,
std::vector<std::vector<std::vector<double> > >& rfiValues,
std::vector<std::vector<std::vector<double> > >& rfiCounts,
std::vector<std::vector<std::vector<double> > > & rfiDists,
std::vector<std::vector<std::vector<double> > > &rfiNs)
...
distance = Tools::getGCDistance(decHold, raHold, decPoint, raPoint,
partSetProcSSS.centerDecDeg)*toDeg;
...
futureFiller.push_back(std::async(std::launch::async,
&Processor::rfiRemovalMulti, this, decHold, raHold, fluxVal,
correlatedScatter));
}
rfiSetFinalValues(futureFiller, rfiWeights, rfiValues, rfiCounts, rfiDists,
rfiNs);
...
void Processor::rfiSetFinalValues(std::vector<std::future<std::vector<double>>>
& futureVec, std::vector<std::vector<std::vector<double> > >& rfiWeights,
std::vector<std::vector<std::vector<double> > >& rfiValues,
std::vector<std::vector<std::vector<double> > >& rfiCounts,
std::vector<std::vector<std::vector<double> > > & rfiDists,
std::vector<std::vector<std::vector<double> > > &rfiNs)
...
std::vector<double> Processor::rfiRemovalMulti(double dec, double ra, double
fluxVal, double correlatedScatter)
{
int  holdI, holdJ, pointCandidates = 0;
...
distance = Tools::getGCDistance(dec, ra, scans[holdI].getDec(holdJ),
scans[holdI].getRa(holdJ), partSetProcSSS.centerDecDeg);
...
avgSigma +=
scatter2dSSS[scans[holdI].getSurveyNumber()][scans[holdI].getScanNumberInSurvey()];
}

avgSigma = (avgSigma / (double)pointCandidates);

rfiRemovePoints(avgSigma, checks, inRange);	//FIT A LOCAL MODEL AND REJECT
MOST DEVIANT POINT UNTIL WITHIN TOLERATED SCATTER rfiFitSolution =
rfiFit(checks, inRange);	//FIT FINAL LOCAL MODEL results =
rfiCollectResults(fluxVal, correlatedScatter, rfiFitSolution, checks, inRange);

return results;
}
std::vector<double> Processor::rfiCollectResults(double fluxVal, double
correlatedScatter, std::vector<double> &rfiFitSolution, std::vector<int> &
checks, std::vector<double> &inRange)
{
bool countAbove, countBelow, rejectPoints, fluxAbove;
double dd;
double distSq, wDistSq, pointCount, wPointCount;
...
double jLMFlux;
//double shapeParam, shapeParamTemp, localModelDumpSum = 0, localModelShapeSum =
0;

//int normalLocalWeightCounter, normalLocalWeight; WHY IS THE normalLocalWeight
AN INTEGER?
...
if (correlatedWeightMap)//We determine the first correction factor and the
correlation scale for photometry here.
{
rfiCountHold = 0.0;
...
dd = scans[jHoldI].getDataDumps(jHoldJ);
pointDistance = Tools::getGCDistance(iDec, iRa, jDec, jRa,
partSetProcSSS.centerDecDeg)*toDeg;//CHECK THAT fwhmRfi*toDeg is right units

if (checks[j] && (pointDistance < fwhmRfi*toDeg) && ((countAbove && fluxAbove)
|| (!countAbove && !fluxAbove))) {//Includes all points that are within a
rfi-beam of LMV && within a rfi-beam of GMV && are (Case 1: above OR Case 2:
below) the 2d-scatter. rfiCountHold += 1; if ((pointDistance != 0))
{
//pointCount += 1.0;
//distSq += pow(pointDistance, 2);
wPointCount += 1.0*dd;
...
{
LMWeight = (dwSum / (1.0 + 2.0 * pow((pw - pwAvg), 2) / (pwDelta /
pwDeltaCounter)));
//wResult = (localModelDumpSum / (1.0 + 2.0 * pow((shapeParam -
localModelShapeSum), 2) / (normalLocalWeight / normalLocalWeightCounter)));
}
...
results.push_back(rfiCountHold);//number of points that satisfy conditions (1),
(2), and (3) in appendix D. (Used to divide local model weights that go into the
global model weight) results.push_back(wDistSq); results.push_back(wPointCount);
...
std::vector<std::vector<double> >
Processor::rfiBuildGlobalModels(std::vector<std::vector<std::vector<double> > >&
rfiWeights, std::vector<std::vector<std::vector<double> > >& rfiValues,
std::vector<std::vector<std::vector<double> > >& rfiCounts,
std::vector<std::vector<std::vector<double> > > & rfiDists,
std::vector<std::vector<std::vector<double> > > &rfiNs)
...
scans[i].setSSSCorrelation(j, 2.0*factor_corr*sqrt(rfiDists[i][j][0] /
std::max(rfiNs[i][j][0], 1.0)));
}
}
...
// theta gap
/*
std::vector<int> Processor::quadrantSort(double dec, double ra, std::vector<int>
&inRange, Quadrant quadrant)
{
        //NOMENCLATURE:
        //RA AND DEC = "x1,y1"
        //RACHECK AND DECCHECK = "x2,y2"

        //THIS FUNCTION WHICH POINTS OF A GIVEN SET SIT WITHIN A CERTAIN
QUADRANT.

        //IT USES RA AND DEC FOR "CARDINAL QUADRANTS" TOP, BOTTOM, LEFT, AND
RIGHT, AND SLOPE FOR "DIAGONAL QUADRANTS"
        ...
        for (int k = 0; k < inRange.size() / 2; k++)
        {
                ...
                if (quadrant == TOP)
                {
                        if ((decCheck > dec) && (raCheck < raLine2) && (raCheck
> raLine))
                        {
...
void Processor::safeguardEdgeThetaGaps(Composite &composite, double weightScale)
{
        ...
        for (int i = 0; i < scans.size(); i++)
        {
                ...
                {
                        scanMapType =
composite.getMapTypes(scans[i].getSurveyNumber()); if (scanMapType != DAISY)
                        {
                                if (((i == 0 || i == scans.size()) || ((j == 0
|| j == scans[i].getSize() - 1)) || (scans[i].getScanNumberInSurvey() == 0 ||
scans[i].getLastScan())))//CHECKS IF FIRST OR LAST SCAN IN SURVEY
                                {
                                        possibles =
findPossSSS(scans[i].getDec(j), scans[i].getRa(j), (4.0
/ 3.0)*std::max(scans[i].getIntraScanGap(), scans[i].getInterScanGap()),
inRangeTemp);//SCAN GAPS IN DEGREES, THETA GAPS IN RADIANS maxLocalThetaGap =
-999999; for (int k = 0; k < possibles; k++)
                                        {
                                                if (maxLocalThetaGap <
scans[inRangeTemp[2 * k]].getThetaGap(inRangeTemp[2 * k + 1]))
                                                {
                                                        maxLocalThetaGap =
scans[inRangeTemp[2 * k]].getThetaGap(inRangeTemp[2 * k + 1]);
                                                }
                                        ...
                                        {
                                                thetaGapFill[inRangeTemp[2 *
k]][inRangeTemp[2 * k + 1]] = maxLocalThetaGap;
                                        }
                                }
                        }
                        else
...
void Processor::determineCircleParams(double dec, double ra, Quadrant quadrant,
std::vector<int> &inRange, std::vector<double> &circleParams)
{
        int scanIndex, dataIndex;
        ...
        kDiag1 = -1.0*1.0;
        kDiag2 = -1.0*-1.0;
        //THE FIRST -1.0 HERE HIGHLIGHTS THAT RA INCREASES RIGHT TO LEFT.
THEREFORE WHAT IS NORMALLY SEEN AS A NEGATIVE SLOPE, IS ACTUALLY A "POSITIVE"
SLOPE IN THIS REFLECTION.
        ...
        for (int k = 0; k < inRange.size() / 2; k++)
        {
                ...
                {
                        y0 = (-1 * pow(x1, 2) + 2 * x1*x2 - pow(x2, 2) + pow(y1,
2) - pow(y2, 2)) / (2 * (y1 - y2)); x0 = x1;
                }
                else if (quadrant == LEFT_QUAD || quadrant == RIGHT_QUAD)
                {
                        x0 = (-1 * pow(x1, 2) + pow(x2, 2) + pow(y1, 2) - 2 *
y1*y2 + pow(y2, 2)) / (-2 * x1 + 2 * x2); y0 = y1;
                }
                else if (quadrant == DIAG_TOP_RIGHT || quadrant ==
DIAG_BOTTOM_LEFT)
                {
                        x0 = -1 * ((-1 * pow(x1, 2) + pow(x2, 2) + 2 * x1*(y1 -
y2) + pow((y1 - y2), 2)) / (2 * (x1 - x2 - y1 + y2))); y0 = (pow(x1, 2) - 2 *
x1*x2 + pow(x2, 2) + 2 * x1*y1 - 2 * x2*y1 - pow(y1, 2) + pow(y2, 2)) / (2 * (x1
- x2 - y1 + y2));
                }
                else if (quadrant == DIAG_BOTTOM_RIGHT || quadrant ==
DIAG_TOP_LEFT)
                {
                        x0 = -1 * ((-1 * pow(x1, 2) + pow(x2, 2) - 2 * x1*(y1 -
y2) + pow((y1 - y2), 2)) / (2 * (x1 - x2 + y1 - y2))); y0 = -1 * ((pow(x1, 2) +
pow(x2, 2) + 2 * x2*y1 - pow(y1, 2) - 2 * x1*(x2 + y1) + pow(y2, 2))) / (2 * (x1
- x2 + y1 - y2));
                }
                ...
                //CircleParameters: scanIndex, dataIndex, distance, radius,
circleCenterRa, circleCenterDec
...
double Processor::maxGapQuadrant(std::vector<double> &circleParams)
{
        ...
        bool thetaGapBool = false, largestInteriorPoint = false, samePoint =
false;
...
circleCenterRa, circleCenterDec
        ...
        Tools::sortAll(0, diameter.size() - 1, diameter, circleCenterRa,
circleCenterDec); Tools::sortAll(0, distance.size() - 1, distance, scanIndices,
dataIndices);
...
TO THE POINT
                        ...
                        raPoint =
scans[scanIndices[iIndex]].getRa(dataIndices[iIndex]); decPoint =
scans[scanIndices[iIndex]].getDec(dataIndices[iIndex]); distToCenter =
Tools::getPythDistance(circleCenterDec[jIndex], circleCenterRa[jIndex],
decPoint, raPoint) + 0.000002;//THIS IS TO HELP FOR WHEN THE DISTANCE TO A POINT
...
{
        std::vector<int> inRangeTemp, inRange, topRange, bottomRange, leftRange,
rightRange, topRightDiagRange, topLeftDiagRange, bottomRightDiagRange,
bottomLeftDiagRange; std::vector<int> scanIndices, dataIndices;
        std::vector<double> circlecircleParams, indicesVec;
        std::vector<double> circleParamsTop, circleParamsBottom,
circleParamsLeft, circleParamsRight; std::vector<double>
circleParamsTopRightDiag, circleParamsTopLeftDiag, circleParamsBottomRightDiag,
circleParamsBottomLeftDiag, circleParamsAll; std::vector<double> diameter,
circleCenterRa, circleCenterDec, distance;
        ...
        double topRightDiagMax, topLeftDiagMax, bottomRightDiagMax,
bottomLeftDiagMax; double topDiagsMax, bottomDiagsMax, diagsMax; double
maxTopBottom, maxLeftRight, quadsMax, maxStandard, maxAll; double thetaGap,
medianStandardGap;
        ...
        if (LSSOn)
        {
                ...
                pointCandidates = findPossLSS(decHold, raHold, psfFWHM,
inRangeTemp);
        }
        else
        {
                ...
                pointCandidates = findPossSSS(decHold, raHold, psfFWHM,
inRangeTemp);
        }
        ...
        //	{
        //		if (inRangeTemp[2 * i] == inRange[2 * j] &&
inRangeTemp[2 * i + 1] == inRange[2 * j + 1])
        //		{
        //			sameNumber = true;
        //		}
        //	}
        ...
        for (int i = 0; i < inRangeTemp.size() / 2; i++)
        {
                distanceCheck = Tools::getGCDistance(decHold, raHold,
scans[inRangeTemp[2 * i]].getDec(inRangeTemp[2 * i + 1]), scans[inRangeTemp[2 *
i]].getRa(inRangeTemp[2 * i + 1]), partSetProcSSS.centerDecDeg)*toDeg;//WE NEED
TO THINK ABOUT IF WE WANT TO TRANSFORM LSS GRID DIFFERENTLY sameNumber = false;
                if (inRangeTemp.size() == 0 && distanceCheck >
scans[i_0].getRCRMinThetaGap(j_0))
                {
                        inRange.push_back(inRangeTemp[2 * i]);
                        inRange.push_back(inRangeTemp[2 * i + 1]);
                }
...
inRangeTemp[2 * i + 1] == inRange[2 * j + 1])
                ...
                if (sameNumber == false && distanceCheck >
scans[i_0].getRCRMinThetaGap(j_0))
                {
                        inRange.push_back(inRangeTemp[2 * i]);
                        inRange.push_back(inRangeTemp[2 * i + 1]);
        ...
        determineCircleParams(decHold, raHold, BOTTOM, bottomRange,
circleParamsBottom); determineCircleParams(decHold, raHold, LEFT_QUAD,
leftRange, circleParamsLeft); determineCircleParams(decHold, raHold, RIGHT_QUAD,
rightRange, circleParamsRight);


        ////DIAGONAL QUADRANTS NE,SE,SW,NW
        topRightDiagRange = quadrantSort(decHold, raHold, inRange,
DIAG_TOP_RIGHT); topLeftDiagRange = quadrantSort(decHold, raHold, inRange,
DIAG_TOP_LEFT); bottomRightDiagRange = quadrantSort(decHold, raHold, inRange,
DIAG_BOTTOM_RIGHT); bottomLeftDiagRange = quadrantSort(decHold, raHold, inRange,
DIAG_BOTTOM_LEFT);

        determineCircleParams(decHold, raHold, DIAG_TOP_RIGHT,
topRightDiagRange, circleParamsTopRightDiag); determineCircleParams(decHold,
raHold, DIAG_TOP_LEFT, topLeftDiagRange, circleParamsTopLeftDiag);
        determineCircleParams(decHold, raHold, DIAG_BOTTOM_RIGHT,
bottomRightDiagRange, circleParamsBottomRightDiag);
        determineCircleParams(decHold, raHold, DIAG_BOTTOM_LEFT,
bottomLeftDiagRange, circleParamsBottomLeftDiag);
