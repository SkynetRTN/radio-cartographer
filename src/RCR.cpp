...
double inverf(double x) {
    ...
    x = log(1 - x * x);
    return squareRootOf2 * sqrt(
               -2.0 / (PI * inverfMult) - x * .5 + sqrt(pow((2.0 / (PI * inverfMult) + x * .5), 2) - x / inverfMult));
}
...
bool isEqual(double x, double y, double maxRelativeError = .00000001, double maxAbsoluteError = DBL_MIN)
...
bool distinctValuesCheck(std::vector<bool> &flags, std::vector<double> &y) {
    ...
    while (i < size) {
        ...
        // if the data point is equal to the true one and is flagged; or  if the data point is not equal and is not flagged
        {
            b = y[i];
            break;
        }
...
//algorithm tools:
void setTrueVec(std::vector<bool> &flags, std::vector<int> &indices, std::vector<double> &w, std::vector<double> &y,
                std::vector<double> &trueW, std::vector<double> &trueY) {
...
void setTrueVec(std::vector<bool> &flags, std::vector<int> &indices, std::vector<double> &y,
...
double getMedian(int trueCount, std::vector<double> &w, std::vector<double> &y) {
    ...
    if (trueCount > 1) {
        ...
        if (sumCounter == 0) {
            ...
            median = y[sumCounter - 1] + (.5 * totalSum - (runningSum - (w[sumCounter - 1] * .5 + w[sumCounter] * .5)))
                     / (w[sumCounter - 1] * .5 + w[sumCounter] * .5) * (y[sumCounter] - y[sumCounter - 1]);
        }
    } else {
        median = y[0];
    }
...
double getMode(int trueCount, std::vector<double> &w, std::vector<double> &y) {
    int k, lowerLimit = 0, upperLimit = trueCount - 1, lowerLimitIn = -1, upperLimitIn = -1, finalLower, finalUpper,
            size;
...
//sigma calculations:
double getOriginFixedRegressionLine(int start, int end, std::vector<double> &w, std::vector<double> &x,
                                    std::vector<double> &y) {
...
double mFinder(int low, int high, int lastXUnderOne, int increment, std::vector<double> &w, std::vector<double> &x,
               std::vector<double> &y) {
    bool stop = false;
...
double get68th(std::vector<double> &w, std::vector<double> &y) {
    ...
    if (y.size() > 1) {
        ...
        if (sumCounter == 0) {
            ...
            stDev = y[sumCounter - 1] + (.682689 * totalSum - (
                                             runningSum - (w[sumCounter - 1] * .317311 + w[sumCounter] * .682689))) / (
                        w[sumCounter - 1] * .317311 + w[sumCounter] * .682689) * (y[sumCounter] - y[sumCounter - 1]);
        }
    } else {
        stDev = y[0];
    }
...
std::vector<double> get2DMode(FunctionalForm &f) {
    ...
    sortedWX = xW;
    sortedWY = yW;
    int xLowIndex = 0, xHighIndex = sortedX.size() - 1, yLowIndex = 0, yHighIndex = sortedY.size() - 1, xLowIndexIn = -1
            , xHighIndexIn = -1, yLowIndexIn = -1, yHighIndexIn = -1;
    double xLowValue, xHighValue, yLowValue, yHighValue;
    bool nullSet = false;
    while (!nullSet && (xLowIndex != xLowIndexIn || xHighIndex != xHighIndexIn || yLowIndex != yLowIndexIn || yHighIndex
                        != yHighIndexIn)) {
...
std::vector<double> get2DMedian(FunctionalForm &f) {
    std::vector<double> sortedX, sortedY, sortedWX, sortedWY, x = f.parameterSpace[0], y = f.parameterSpace[1], xW = f.
            weightSpace[0], yW = f.weightSpace[1];
    sortedX = x;
    sortedY = y;
    sortedWX = xW;
    sortedWY = yW;
...
std::vector<double> get3DMode(FunctionalForm &f) {
    ...
    sortedYW = yw;
    sortedZW = zw;
    int xLowIndex = 0, xHighIndex = sortedX.size() - 1, yLowIndex = 0, yHighIndex = sortedY.size() - 1, zLowIndex = 0,
            zHighIndex = sortedZ.size() - 1;
    ...
    bool nullSet = false;
    while (!nullSet && (xLowIndex != xLowIndexIn || xHighIndex != xHighIndexIn || yLowIndex != yLowIndexIn || yHighIndex
                        != yHighIndexIn || zLowIndex != zLowIndexIn || zHighIndex != zHighIndexIn)) {
        xLowIndexIn = xLowIndex;
        xHighIndexIn = xHighIndex;
        yLowIndexIn = yLowIndex;
        yHighIndexIn = yHighIndex;
        ...
        for (size_t i = 0; i < x.size(); i++) {
            if (x[i] >= xLowValue && x[i] <= xHighValue && y[i] >= yLowValue && y[i] <= yHighValue && z[i] >= zLowValue
                && z[i] <= zHighValue) {
                sortedX.push_back(x[i]);
                sortedY.push_back(y[i]);
    ...
    sort(sortedZW, sortedZ);
    std::vector<double> toRet;
    double b = getMedian(sortedX.size(), sortedXW, sortedX), m = getMedian(sortedY.size(), sortedYW, sortedY), xNot =
            getMedian(sortedZ.size(), sortedZW, sortedZ);
...
std::vector<double> get3DMedian(FunctionalForm &f) {
    std::vector<double> sortedX, sortedY, sortedZ, x = f.parameterSpace[0], y = f.parameterSpace[1], z = f.
            parameterSpace[2];
    std::vector<double> sortedXW, sortedYW, sortedZW, xw = f.weightSpace[0], yw = f.weightSpace[1], zw = f.weightSpace[
        2];
...
std::vector<double> w, y, sortedW, sortedY, halfSampleSubsetOld, halfSampleSubsetNew;
std::vector<std::vector<double> > inBoundsParameterSpace, inBoundsWeightSpace;
struct Bound
{
...
Bound paramBound;
std::vector<Bound> halfSampleBounds;

inBoundsParameterSpace = f.parameterSpace;
inBoundsWeightSpace = f.weightSpace;

for (int i = 0; i < f.parameterSpace.size(); i++)
{
y = inBoundsParameterSpace[i];
w = inBoundsWeightSpace[i];
sortedY = y;
sortedW = w;
...
void RCR::setInitialModel(std::vector<double> &y) {
    ...
    while (!found) {
        ...
        for (size_t i = 0; i < models.size(); i++) {
            if (std::abs(models[i][0] - model[0]) < .0001 && std::abs(models[i][1] - model[1]) < .0001)
            // && std::abs(models[i][2] - model[2]) < .0001)
            {
                found = true;
                indexFound = i;
            }
...
double RCR::getLower68CF(int n, std::vector<double> &w) {
    ...
    if (n == 2) {
        ...
        b1 = -1.4528 * pow(logn, 3) + 5.3519 * pow(logn, 2) - 5.33 * logn + 2.2902 + pow(-1, n) * 0.1879 * pow(
                 logn, 0.9521);
...
double RCR::getLowerDLCF(int n, std::vector<double> &w) {
    ...
    if (n == 2) {
        ...
        b1 = 43.179 * pow(logn, 6) - 331.85 * pow(logn, 5) + 968.25 * pow(logn, 4) - 1399.1 * pow(logn, 3) + 1070.7 *
             pow(logn, 2) - 415.81 * logn + 65.002 + pow(-1, n) * 0.1365 * pow(logn, 2.4716);
...
double RCR::getSingleFN(int n, std::vector<double> &x, std::vector<double> &w) {
    ...
    if (3 < n && n < 8) {
        ...
        b1 = -0.3556 * pow(logn, 6) + 3.7036 * pow(logn, 5) - 14.932 * pow(logn, 4) + 29.176 * pow(logn, 3) - 28.81 *
             pow(logn, 2) + 14.397 * logn - 2.6451;
        a1 = 0.2313 * pow(logn, 6) - 3.02 * pow(logn, 5) + 15.997 * pow(logn, 4) - 43.713 * pow(logn, 3) + 64.629 *
             pow(logn, 2) - 49.976 * logn + 15.484 + pow(-1, n) * 0.1513 * pow(n, -0.471);
        f_n = y1 * pow(10, pow(10, a1 + b1 * logx));
    } else if (n > 1000) {
        f_n = y1;
    } else {
...
double RCR::getLowerFN(int n, std::vector<double> &x, std::vector<double> &w) {
    ...
    if (n == 5) {
        ...
        a1 = -0.541 * pow(logn, 5.0) + 4.6943 * pow(logn, 4.0) - 15.407 * pow(logn, 3.0) + 21.875 * pow(logn, 2.0) -
             11.211 * logn - 0.3798;
...
double RCR::getEachFN(int n, std::vector<double> &x, std::vector<double> &w) {
    ...
    if (n == 5) {
        ...
        a1 = 3.1767 * pow(logn, 6.0) - 34.561 * pow(logn, 5.0) + 152.16 * pow(logn, 4.0) - 347.96 * pow(logn, 3.0) +
             435.59 * pow(logn, 2.0) - 282.57 * logn + 73.696;
...
//rejectors:
bool RCR::reject(int paramCount, int trueCount, int &index, double max, std::vector<bool> &flags,
                 std::vector<double> &y) {
...
bool RCR::bulkReject(int paramCount, std::vector<bool> &flags, std::vector<int> &index, std::vector<double> &max,
                     std::vector<double> &y) {
...
bool RCR::bulkReject(std::vector<bool> &flags, std::vector<int> &index, std::vector<double> &max,
                     std::vector<double> &y) {
    bool noPointsRejected = true, threeDistinct = true;
    int size = max.size();
    int i = size - 1;
...
double RCR::fitDL(double counter, std::vector<double> &w, std::vector<double> &x, std::vector<double> &y) {
    ...
    double a = 0, b = 0, c = 0, d = 0, e = 0, f = 0, doubleLineError = 0, singleLineError = 0, singleLineFit, xAtM,
            xAtIxAtMDiff, wxProd, sigma, tau, factor, deltaChiSquared, errorAtI;
    ...
    int m = (int) (mFinder(1, amountXUnderOne - 1, amountXUnderOne - 1, (int) (max((double) (y.size()) / 6.36, 1.0)), w,
                           x, y));
    xAtM = x[m];
    for (int i = 0; i <= m; i++) {
        wxProd = w[i] * x[i];
...
double RCR::fitDL(double counter, std::vector<double> &x, std::vector<double> &y) {
    int amountXUnderOne = 0;
    double a = 0, b = 0, c = 0, d = 0, e = 0, f = 0, doubleLineError = 0, singleLineError = 0, singleLineFit, xAtM,
            xAtIxAtMDiff, sigma, tau, factor, deltaChiSquared, errorAtI;
    amountXUnderOne = countAmountLessThanOne(x);
    singleLineFit = getOriginFixedRegressionLine(0, amountXUnderOne, x, y);
    if (x.size() < 4) {
        return singleLineFit;
    }
    int m = (int) (mFinder(1, amountXUnderOne - 1, amountXUnderOne - 1, (int) (max((double) (y.size()) / 6.36, 1.0)), x,
                           y));
    xAtM = x[m];
    for (int i = 0; i <= m; i++) {
        a += x[i] * x[i];
...
void RCR::iterativeSingleSigmaRCR(std::vector<double> &w, std::vector<double> &y) {
    ...
    double mu, stDev, sigma, hold, max;
    max = -99999;
    ...
    while (!stop) {
        ...
        if (this->muType == PARAMETRIC) {
            stop = reject(this->parametricModel->parameterSpace.size(), trueCount, maxIndex, max / sigma, localFlags,
                          trueY);
        } else {
...
void RCR::bulkLowerSigmaRCR(std::vector<double> &w, std::vector<double> &y) {
    ...
    while (!stop) {
        ...
        if (nonzeroAbove && nonzeroBelow) {
            sigma = min(handleBulkSigmaTechSelect(trueCount, wBelow, diffBelow),
                        handleBulkSigmaTechSelect(trueCount, wAbove, diffAbove)) * nCorrect(trueCount, trueW);
        } else if (nonzeroAbove) {
...
void RCR::bulkLowerSigmaRCR(std::vector<double> &y) {
    ...
    while (!stop) {
        ...
        if (nonzeroAbove && nonzeroBelow) {
            sigma = min(handleBulkSigmaTechSelect(trueCount, wBelow, diffBelow),
                        handleBulkSigmaTechSelect(trueCount, wAbove, diffAbove)) * nCorrect(trueCount);
        } else if (nonzeroAbove) {
