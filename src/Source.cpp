...
double setRFIMaxVal(SurveyParameters sParams, int MJD) {
  // WE NEED EXCEPTION HANDLING FOR FREQUENCIES/CONFIGURATIONS THAT DON'T ALIGN
  // WITH THESE FREQUENCIES RFI Max can't be used for multiple surveys with
  // different dates yet. RFIMax() needs exception handling for different
  // configurations.
  ...
  if (sParams.tele == FOURTY_FOOT) {
    ...
    } else if ((sParams.frequency == 1.680 || sParams.frequency == 1.700) &&
               MJD < 56870) {
      if (sParams.channel == COMPOSITE) {
        maxRFI = 1.1;
      } else {
...
void setInputPhotoParams(char *argv[], PhotoParams &pParams) {
  ...
  // pParams.coordinatesPixels = { 103, 21, 231, 61, 153, 161, 134, 182, 385,
  // 196, 331, 214, 245, 258, 201, 267, 285, 360, 299, 409 };
}
void setInputSpectralParams(int argc, char *argv[],
                            SpectralParameters &cParams) {
  // IN-PROGRESS
  cParams.subScale = 0.0;
  cParams.modSubScale = 0.0;
  cParams.modSubZones =
      {}; //{1423.929214, 1424.051278, 1424.066536, 1424.219116, 1424.234374,
          // 1424.310664};// {1435.0, 1447.5, 1522.5, 1630.0};
  cParams.receiver = HI;
  if (atoi(argv[3]) == 2)
    cParams.receiver = LO;
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  //    Survey survey2(sParams, cParams,
  //    "/skynet/radio-cartographer/testing/Temporary/barnard.fits");
  ...
  // Mapping Enums/Structs to Strings/Ints
  headerInfo["RCCHAN"] =
      std::to_string(sParams.channel); // 0=LEFT, 1=RIGHT, 2=COMPOSITE
  headerInfo["RCSPEC"] = std::to_string(
      cParams.receiver); // 0=HI, 1=LO (Need to check enum values)
  headerInfo["RCCALI"] = std::to_string(sParams.calMethod);
  headerInfo["RCCORDS"] = std::to_string(sParams.pCoordinate);
  ...
  headerInfo["RCBG"] = std::to_string(procParams.bgScaleBW);
  headerInfo["RCRFI"] = std::to_string(procParams.rfiScaleBW);
  headerInfo["RCM10"] = std::to_string(
      procParams.rfiScaleBW); // Script used rfiScale for M10 too?
  headerInfo["RCWGT"] = std::to_string(procParams.wScaleBW);
  headerInfo["RCPHOT"] = std::to_string(pParams.perform);
  headerInfo["RCAPT"] =
      std::to_string(pParams.innerRadius); // Assuming innerRadius = aperture
  headerInfo["RCANL"] =
      std::to_string(pParams.outerRadius); // Assuming outerRadius = annulus
  headerInfo["RCSFM"] = std::to_string(pParams.centroidType);

  // Coordinates? Script used job.coordinates. Here we don't have explicit
  // source coords unless photometry headerInfo["RCCORD"] = ...

  headerInfo["RCTRIM"] = std::to_string(sParams.trimSize);
  headerInfo["RCLSS"] = std::to_string(mParams.LSSMapping);
  ...
  // EXIT
  std::cout << "The code has exited successfully with return code 0"
            << std::endl;
