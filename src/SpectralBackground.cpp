...
std::vector<double> spectralClean(SpectralBaseline &baseline)
{
...
SpectralBaseline::SpectralBaseline(bool forward, double scatter, double xAnchor, double yAnchor, std::vector<bool> &checks, std::vector<double> &flux, std::vector<double> &dataDumps, std::vector<double> &angDist)
{
...
SpectralBaseline::SpectralBaseline()
{
}
SpectralBaseline::~SpectralBaseline()
{
...
int SpectralBaseline::sufficentPointCheck()
{
...
bool SpectralBaseline::findDuplicate(double value, std::vector<double> vectorHold)
{
...
std::vector<double> SpectralBaseline::autoFixedWRegression()
{
...
std::vector<double> SpectralBaseline::autoWRegression()
{
    // ATTEMPTS QUADRATIC REGRESSION
    // IF A COEFFICENT IS UNSTABLE RESORST TO LINEAR
    // IF LINEAR FALIS NO MODEL
...
std::vector<double> SpectralBaseline::applyModel(double x, std::vector<double> &coefs)
{
...
std::vector<double> SpectralBaseline::getResults()
{
    return this->BLResults;
}

double SpectralBaseline::rejectPoints()
{
...
int SpectralBaseline::returnPoints()
{
    if (forward)
    {
        BLChecks[0] = false;
...
void SpectralBaseline::setLocalModels()
{
...
SpectralBackground::SpectralBackground(const std::vector<double>& inputAngDist, const std::vector<double>& inputFlux, const std::vector<double>& inputDataDumps, double inputScatter)
{
    size = inputFlux.size();
    angDist = inputAngDist;
    flux = inputFlux;
    dataDumps = inputDataDumps;
    scatter = inputScatter;
}
SpectralBackground::~SpectralBackground()
{
}

std::vector<double> SpectralBackground::calculateBG(double baseline)
{
    SpectralBaseline baselineTemp;
    int start;
    ...
    std::vector<std::vector<double>> cleanSpectralBaselineArray;
    cleanSpectralBaselineArray.resize(baselineArray.size());

    for (int i = 0; i < baselineArray.size(); i++)
    {
        cleanSpectralBaselineArray[i] = spectralClean(std::ref(baselineArray[i]));
    }
    for (int i = 0; i < indicesForward.size() / 2; i++)
    {
        start = indicesForward[2 * i];
        baselineLM = cleanSpectralBaselineArray[i];
        loadLocalModels(baselineLM, start);
    }
    for (int i = 0; i < indicesBackward.size() / 2; i++)
    {
        start = indicesBackward[2 * i];
        baselineLM = cleanSpectralBaselineArray[(indicesForward.size() / 2) + i];
        loadLocalModels(baselineLM, start);
    }
...
std::vector<double> SpectralBackground::calculateBGMulti(double baseline)
{
    SpectralBaseline baselineTemp;
    int start;
    std::vector<int> indicesForward, indicesBackward;
    ...
    for (int i = 0; i < baselineArray.size(); i++)
    {
        futureVec[i] = std::async(std::launch::async, spectralClean, std::ref(baselineArray[i]));
    }
    for (int i = 0; i < indicesForward.size() / 2; i++)
    {
        start = indicesForward[2 * i];
...
void SpectralBackground::buildBaselines(bool forward, std::vector<int> &indices)
{
    int start, end;
    ...
    for (int i = 0; i < indices.size() / 2; i++)
    {
        ...
        baselineTemp = SpectralBaseline(forward, scatter, xAnchor, yAnchor, checksTemp, zTemp, dumpsTemp, angDistTemp);
        baselineArray.push_back(baselineTemp);
    }
}
void SpectralBackground::loadLocalModels(std::vector<double> &results, int start)
{
    int index;
...
std::vector<std::vector<int>> SpectralBackground::findStartEndIndices(std::vector<double> &angDist, double baseline)
{
...
std::vector<double> SpectralBackground::setBackground(std::vector<std::vector<double>> &bgData, std::vector<std::vector<double>> &bgWeights)
{
    double low, high;
    int trueCount;
