...
private:
  ...
  std::vector<double> determinePercentDrop(std::vector<int> &,
                                           std::vector<int> &);
  ...
  void rejectModelPoints(int, int, double, double, double,
                         std::vector<double> &, std::vector<bool> &);
  double lssRecursivePolyFit(Survey &, int, int, double, double);
  bool planeApplication(int, int, int, std::vector<std::vector<int>>, int,
                        MapTypes);
  int correctIndex(int, int, int, int);
  std::vector<std::vector<int>>
  calculateScanNumbers(int &, std::vector<double> &, std::vector<bool> &);
  // int lssFindPoss(double, double, double, double, std::vector<int> &);
  bool verifyOutlierState(SurfaceType, int, int, double, std::vector<double> &,
                          std::vector<double> &);
  double calculateSecondDerivitive(SurfaceType, int, int, double,
                                   std::vector<double> &);
  double calculateThetaPerp(int, int);
  double applySurfaceModel(double, double, double, std::vector<double> &);
  std::vector<double> LSSCollectResults(int, int, double, std::vector<double> &,
                                        std::vector<double> &,
                                        std::vector<bool> &,
                                        std::vector<double> &);

  SurfaceType determineSurfaceType(std::vector<std::vector<int>> &, MapTypes);
  std::vector<double> determineModelCoef(SurfaceType, int,
                                         std::vector<double> &,
                                         std::vector<bool> &, double, double,
                                         double);
  std::vector<double> surfaceFit3(std::vector<double> &, std::vector<bool> &,
                                  double, double, double);
  std::vector<double> surfaceFit6(std::vector<double> &, std::vector<bool> &,
                                  double, double, double);
  std::vector<double> surfaceFit10(std::vector<double> &, std::vector<bool> &,
                                   double, double, double);

  bool checkEdgeCriteria(Coordinates, PartitionSet, double, double);
  std::vector<double> determineMappedEdges(double, double, bool, PartitionSet,
                                           Coordinates, MapTypes);

  double surfaceModel(std::vector<Scan>, PartitionSet, double, double, double,
                      double);
